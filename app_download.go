...
func (a *App) downloadFileWithQueue(rawURL, saveDir string) (string, error) {
	...
	if customURL := rewriteMirrorURL(cfg, rawURL); customURL != "" && customURL != rawURL {
	...
	if mirror == "jsdelivr" {
		prefer = "jsd"
	} else if mirror == "githubapi" {
		prefer = "api"
	} else if strings.HasPrefix(mirror, "custom-") {
		prefer = "custom"
	}
	if prefer != "" {
		for i := range sources {
			if sources[i].kind == prefer {
