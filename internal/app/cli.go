...
.summary b{font-size:18px}
		...
		sb.WriteString(fmt.Sprintf("<div class=meta><span>📁 %s</span><span>🦴 %d 骨骼</span><span>📦 %d 立方体</span><span>🖼️ %dx%d</span></div>",
			html.EscapeString(filepath.Base(f)), model.BoneCount, model.CubeCount, model.TexWidth, model.TexHeight))
		...
		for _, iss := range issues {
			...
			sb.WriteString(fmt.Sprintf("<div class=\"issue %s\">%s <span class=cat>[%s]</span>%s</div>", cls, icon, html.EscapeString(iss.Category), html.EscapeString(iss.Message)))
		}
