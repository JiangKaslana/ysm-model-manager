...
// 复用 loader.ts 的 MMD 子目录回溯逻辑：GetRepoRoot("mmd-skin") 返回
// FilesRoot/mmd/EntityPlayer，回溯到 group 根再拼目标子目录，产出绝对路径。
// 与导航栏 MMD_SUBTYPES 声明同源，消除"用户自选目录"带来的路径漂移。

import { RESOURCE_TYPES } from "../../resource/types.ts";

/** MMD 动作库子目录名（与 MMD_SUBTYPES 声明一致） */
 ...
 *
 * @param repoRoot GetRepoRoot("mmd-skin") 返回的路径，如 "FilesRoot/mmd/EntityPlayer"
