...
//   1) 程序化 Canvas 2D → Equirectangular 纹理（EquirectangularReflectionMapping 必设置）
...
//   4) dispose 时 dispose PMREM 产物并还原 scene.environment
// 不用 HDR 文件零外部资源，避免 CDN/hdr 文件路径问题；后期可加 RGBELoader 增强。
...
export class EnvironmentCapability implements SceneCapability {
  ...
  private buildEnvironment(): void {
    ...
    const preset = ENV_PRESETS[this.params.preset] ?? ENV_PRESETS.sky;
  ...
  setPreset(modelType: string): void {
    ...
    this.params = { ...this.params, ...modelPreset };
    this.buildEnvironment();
  }
  ...
  setIntensity(v: number): void {
    ...
    // 影响材质：遍历场景里的内容层 mesh（用户触发调整时，scan scene 全部 mesh；
    // 下次 build/switchTo 时 syncMeshIntensity 再增量同步）。
    const roots: THREE.Object3D[] = [];
    this.scene.traverse((obj) => {
      if ((obj as THREE.Mesh).isMesh) roots.push(obj.parent ?? obj);
    });
    // 只去重根节点（取所有 mesh 的最顶层公共父：简化版直接对每个 mesh 赋值即可）
  ...
  getMenuControls(): MenuControlDef[] {
    const options: Array<{ value: string; label: string }> = (Object.keys(ENV_PRESETS) as EnvPresetId[]).map(
  ...
  loadState(): void {
    ...
    if (state.preset && ENV_PRESETS[state.preset as EnvPresetId]) this.params.preset = state.preset as EnvPresetId;
