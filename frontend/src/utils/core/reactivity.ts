// utils/core/reactivity.ts — 轻量响应式系统
// Proxy 拦截 set → RAF 去抖 → 通知所有订阅者（携带变更键信息），实现精确增量刷新。
//
// 订阅者签名：接收变更键集合 `(changedKeys: Set<string>) => void`。
...
/**
 * 清空所有刷新订阅者。供场景/视图重入时调用，避免旧订阅者泄漏。
 */
 ...
 * 或通过整体赋值。Map/Set 同理：替换整个实例，不依赖内部变更触发刷新。
 */
const _proxyCache = new WeakMap<object, object>(); // 缓存已创建的 Proxy，保证引用稳定
export function reactive<T extends object>(obj: T): T {
    ...
    const proxy = new Proxy(obj, {
        ...
        set(target, key, value, receiver): boolean {
            ...
            // 记录被修改的顶层 key，供 scheduleRefresh 传递给订阅者
