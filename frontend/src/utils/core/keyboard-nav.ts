...
export interface KeyboardNavOptions {
     ...
     * 由外部负责打 CSS 类 + scrollIntoView + focus。
     */
    setActiveIndex?: (items: HTMLElement[], nextIdx: number) => void;
    /**
     * ArrowRight 语义为「激活当前项」而非「平级向后移动」（层级进入）。
     ...
     * 用于保证焦点真相源（getActiveIndex 的索引）与 list.length 完全一致，避免 wrap 边界错位。
     */
...
export function createKeyboardNav(
    ...
    const handler = (e: KeyboardEvent) => {
        ...
        // 焦点真相源：优先外部 getActiveIndex，否则回退原生 `:focus` 反查。
        ...
        switch (e.key) {
            ...
            case "ArrowRight": {
                ...
                // 语义：ArrowRight = 激活当前项（层级进入），非平级移动
...
function _moveFocus(
    ...
    // 焦点真相源写入：外部提供 setActiveIndex 时由其接管，否则走默认 roving tabindex + 原生 focus。
