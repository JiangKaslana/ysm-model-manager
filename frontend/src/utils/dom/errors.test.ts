...
describe("friendlyError 结构化 AppError Code（ADR-051）", () => {
  it("err.cause.Code 已知 → 映射 i18n（Wails RuntimeError 形状）", () => {
    ...
    expect(friendlyError(err)).toBe("文件已存在");
  });

  it("err.Code 已知（兼容兜底形状）→ 映射 i18n", () => {
    expect(friendlyError({ Code: "UNSUPPORTED_FORMAT" })).toBe("不支持的格式或操作");
    expect(friendlyError({ Code: "INVALID_PARAM" })).toBe("参数无效");
  });

  it("未知 Code → 回退 message/正则路径（不静默）", () => {
    expect(friendlyError({ Code: "WRITE_FAILED", message: "写入失败: boom" })).toBe(
      "写入失败: boom",
    );
  });

  it("未列出的具体 Code（FILE_TOO_LARGE/FILE_EMPTY/LINK_FAILED）→ 中文 Reason 透传，不武断归类", () => {
    // 这些 Code 的 Reason/Suggestion 比通用分类更具体（如「文件大小超过 500MB 限制」），
    // 映射到 unsupported/fileLocked 会误导用户——ADR-051 决策靠 Reason 中文透传
