...
export function initImportQueue(app: ImportQueueHost): () => void {
  ...
  // P3 修复（子代理审计）：readAndRouteFile 的 FileReader/直导均为异步回调，
  // cleanup 只清 timer/监听不阻断在途回调 → 用 disposed 标志丢弃 init 销毁后
  // 迟到的结果（与 recycle-bin/_loadGen 同思路）
  let disposed = false;
  // 读文件并分流（表单/直导）+ 可选完成回调
  ...
  const showForm = (file: ImportFile, base64: string): void => {
    ...
    // P3 修复（子代理审计）：timer 引用保存供 cleanup 清理——原匿名 setTimeout 不保留
    // id，cleanup 后仍会触发，读取已脱离 DOM 的 root（资源未释放缺口）
    ...
    pendingHeaderTimers.push(headerTimer);
  };

  // 检查文件是否已存在（防抖）
  let conflictTimer: ReturnType<typeof setTimeout> | null = null;
  // P3 修复（子代理审计）：showForm 内 auto-header 定时器集合——cleanup 统一清理，
  // 防组件销毁后读取已脱离 DOM 的 root
  ...
  on(dropZone, "drop", (e: DragEvent) => {
    ...
    if (resolveWebMode()) {
      ...
      if (!files?.length) {
        // 拖入文件夹时 dataTransfer.files 为空 → 明确提示而非静默 return（P3 修复）
        bus.emit("toast:show", {
      ...
      // ADR-044① 异步 handler 最外层必有 catch 出口（当前 importWebFiles 吞错，但
  ...
  on(fileInput, "change", () => {
    ...
    // （P2 修复：此前走桌面 readAndRouteFile → ImportModelFileTo fail-fast 抛错）
    if (resolveWebMode()) {
      void (async () => {
        try {
          const r = await importWebFiles(Array.from(files), "ysm");
  ...
  if (importBtn)
    ...
    // 与 updatePreview 同口径，消除预览/实际文件名漂移
    const d =
      manualDate ||
      (autoOn
        ? new Date().getFullYear() +
  ...
  const enqueueFile = (file: ImportFile, base64: string): void => {
    ...
    // P3 修复（子代理审计）：去重键大小写敏感（foo.ysm vs FOO.ysm 双入队）+
    // relPath 形态不一致（直导历史条目 relPath 为 undefined、表单路径为 ""，
    // `undefined === ""` 恒 false → 直导条目永远不参与去重）——统一
    // `name.toLowerCase()` + `relPath || ""` 归一比较
    const relPath = file._relPath || "";
    const nameKey = file.name.toLowerCase();
    const dup =
      fileQueue.some((fq) => fq.name.toLowerCase() === nameKey && (fq.relPath || "") === relPath) ||
  ...
  return () => {
    disposed = true;
    if (conflictTimer) clearTimeout(conflictTimer);
    // P3 修复（子代理审计）：清理 auto-header 定时器（原遗漏，cleanup 后仍会触发）
