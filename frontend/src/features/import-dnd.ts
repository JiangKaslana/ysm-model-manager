...
const onDrop = async (e: DragEvent): Promise<void> => {
  ...
  // （browserAdapter，含 IDB 降级内存 Map）。桌面/Android 走下方 collectFiles 桌面流。
  if (resolveWebMode()) {
    const files = Array.from(e.dataTransfer?.files || []);
    if (files.length === 0) {
      // P2 修复（审核发现）：拖入文件夹时 dataTransfer.files 为空，此前静默 return
      // 零反馈；对齐 import-queue.ts 同场景的 warn toast（双路径反馈一致）
      bus.emit("toast:show", {
  ...
  // P2 修复（子代理审计）：去重键弃用 File 对象身份（entry.file() 返回新建实例，
  // 与 dataTransfer.files 的身份不保证相等——两条路径同时命中时同一文件被收集两次
  // → 顺序直导两次 → 双导入）；改 name+size+lastModified 稳定键
  ...
  // P2/P3 修复（子代理审计）：oversize 改逐文件过滤——原整批 abort（任一文件 >100MB
  // 就 return，同批合法文件全部不导入，仅报第一个超限名）；与网页分支 importWebFiles
  // （browser-adapter.ts:253 逐文件跳过）及导入页 drop 路径行为不一致。
  // 现剔除超限文件继续导入合法项，warn toast 列出被跳过文件名
