...
export function bindRepoEvents(
  ...
  if (dlSelBtn) {
    dlSelBtn.addEventListener("click", async () => {
      // P3 修复（子代理审计）：busy 命中不再静默 return——与单文件路径
      // （L232-236 busy 发 toast）防御范式一致：用户反复点击时知道为何无响应；
      // 空选中集保持静默（无操作对象，属正常态）
      ...
      try {
        ...
        // buildDownloadTasks/enqueue 抛错原逸出为 unhandled rejection
        bus.emit("toast:show", {
  ...
  if (dlContainer) {
    dlContainer.addEventListener("click", async (e: MouseEvent) => {
      try {
        ...
        // P2 修复（审核）：事件委托 handler 最外层 catch 出口（ADR-044 ①）——
        // handleSingleDownload 内 enqueue reject 原经 finally 逸出为 unhandled rejection
        bus.emit("toast:show", {
