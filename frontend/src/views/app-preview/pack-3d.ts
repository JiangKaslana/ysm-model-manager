// ===== pack-3d.ts — MC 资源包模型 3D 预览薄包装（ADR-080）=====
// 内容层在 pack-model-adapter.ts；本文件仅负责 getApp 注入 Go 绑定 + 公开符号
...
function makePackDeps() {
  return {
    listModels: async (path: string): Promise<string[]> => {
...
/** 打开资源包模型 3D 预览（无 block 模型时 build 抛错，调用方回退缩略图） */
export async function createPack3D(path: string, opts?: Mount3DOptions): Promise<void> {
  await mount3D(makePackAdapter(makePackDeps()), path, opts);
}
