...
export function sidebarHTML(
  ...
  if (actions.includes("import")) {
    btns +=
      '<button class="btn-base sm rm-import-btn" data-testid="rm-import" style="flex:1">📥 ' + t("common.import") + '</button>';
  }
  ...
  html +=
    '<input class="rm-search" data-testid="rm-search" type="text" placeholder="🔍 ' +
    t("rm.search", { label: esc(label) }) +
    ...
    '<div class="rm-list" style="flex:1;overflow-y:auto"><div style="padding:12px;text-align:center;color:var(--muted)">' +
    t("common.loading") +
    "</div></div>" +
    "</div>";
  return html;
...
export function detailHTML(
  ...
  if (meta.thumbnail) {
    ...
    html +=
      ...
      '<div style="font-size:var(--fs-sm);color:var(--muted)">' + t("rm.noPackThumb") + "</div>" +
      '</div>';
  }
  html +=
  ...
  html +=
    '<div style="color:var(--muted);font-size:var(--fs-xs)">' +
    t("rm.packFormat", { format: esc(rv.format) }) +
    (rv.version ? "（" + esc(rv.version) + "）" : "") +
    "</div>" +
    '<div style="color:var(--muted);font-size:var(--fs-xs)">' +
    t("rm.status") +
    ": " +
    (enabled ? t("rm.enabled") : t("rm.disabled")) +
    "</div>" +
  ...
  if (actions.includes("delete")) {
    html +=
      '<button class="btn-base danger sm rm-del-btn" style="align-self:flex-start;margin-top:8px">🗑️ ' +
      t("rm.deleteLabel", { label: esc(label) }) +
      "</button>";
  }
  html += "</div>";
  return html;
...
export function placeholderHTML(label: string): string {
  return (
    ...
    "<div>" +
    t("rm.selectToView", { label: esc(label) }) +
    "</div>" +
    "</div>"
  );
}
