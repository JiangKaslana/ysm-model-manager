  ...
  if (themePicker) {
    themePicker.querySelectorAll(".theme-card").forEach((card) => {
      ...
      card.addEventListener("click", () => {
        ...
        // L31 的 const linkMode 是 initSettings 捕获值，用户在链接模式下拉改过后不更新，
        // 主题切换会用旧值把已改的 linkMode 覆盖回退
        void (async () => {
          try { await SaveAppConfig(cfg.filesRoot || "", cfg.resourcepackRoot || "", cfg.mcRoot || "", cfg.linkMode || "copy", themeName); }
          catch (e) { console.warn("[settings] 主题保存到配置失败:", e); /* 保存失败不影响 UI 主题，但留痕便于排障 */ }
        })();
        // 关闭自动切换
