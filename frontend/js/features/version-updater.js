...
async function promptUpdate(info, statusEl) {
  const ok = await new Promise((resolve) => {
    ...
    box.className = "dlg-box dlg-pad update-modal";
    box.style.cssText = "gap:10px;width:520px";
    ...
    const notesHTML = info.releaseNotes
      ? (() => {
          ...
          d.textContent = raw;
          return `<div class="update-notes" style="border:1px solid var(--bd);border-radius:6px;background:var(--bg);padding:10px;font-size:11px;line-height:1.6;white-space:pre-wrap;max-height:40vh;overflow-y:auto;color:var(--txt);margin-top:4px">${d.innerHTML}</div>`;
        })()
      : "";

    box.innerHTML = `
      <div class="dlg-title update-title" style="margin:0">📦 发现新版本</div>
      <div class="update-summary" style="font-size:12px;color:var(--txt);line-height:1.5">发现新版本 <strong>${esc(info.latest)}</strong>（当前 ${esc(info.current)}）<br>是否下载并更新？</div>
      ${notesHTML ? `<div class="update-notes-label" style="font-size:11px;color:var(--muted);margin-top:6px">━━━ 更新日志 ━━━</div>${notesHTML}` : ""}
      <div class="dlg-footer" style="padding:0">
        <button class="dlg-btn update-cancel" id="um-cancel">取消 (Esc)</button>
        <button class="dlg-btn dlg-btn-primary update-confirm" id="um-ok">⬇️ 下载更新</button>
      </div>
    `;
