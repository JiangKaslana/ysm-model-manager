...
export function initImportQueue(app) {
  ...
  root.getElementById("dl-preview-model")?.addEventListener("click", async () => {
    if (!currentBase64) {
      bus.emit("toast:show", {
        msg: "请先选择一个模型文件",
        duration: 2000,
    ...
    try {
      const { SavePreviewTempFile } =
        await import("../../wailsjs/go/main/App.js");
      const tmpPath = await SavePreviewTempFile(currentBase64);
      if (tmpPath) {
        bus.emit("model:select", { path: tmpPath });
      }
    } catch (err) {
      bus.emit("toast:show", {
        msg: "预览失败: " + friendlyError(err),
        duration: 3000,
        type: "error",
      });
    }
  ...
  const renderImportedList = () => {
    ...
    imported.forEach((item) => {
      html +=
        '<div class="dl-imported-item" style="display:flex;align-items:center;gap:4px;padding:2px 4px;border-radius:3px;font-size:10px;border:1px solid var(--bd)">' +
        '<span class="dl-item-name" style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--txt)">' +
        esc(item.name) +
        "</span>" +
        '<span class="dl-item-time" style="font-size:9px;color:var(--muted);flex-shrink:0">' +
        (item.time || "") +
        "</span>" +
    ...
    fileQueue.forEach((fq, qi) => {
      ...
      html +=
        ...
        ';cursor:pointer">' +
        '<span class="dl-q-status" style="color:var(--muted);font-size:9px">' +
        (isEditing
          ? "✏️"
