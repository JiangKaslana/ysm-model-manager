...
<div class="recy-row" style="display:flex;align-items:center;gap:6px">
<span class="recy-name" style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--txt)" title="${esc(e.Path)}">${renderDisplayName(name)}</span>
<span class="recy-size" style="font-size:var(--fs-xs);color:var(--muted)">${size}</span>
<button class="recy-preview" data-preview-path="${esc(e.Path)}" style="padding:2px 6px;border-radius:3px;border:1px solid var(--bd);background:var(--surf);color:var(--txt);cursor:pointer;font-size:var(--fs-xs)">👁 预览</button>
...
<div class="recy-path" style="font-size:var(--fs-xs);color:var(--muted);padding-left:2px;word-break:break-all">📂 ${esc(e.Path)}</div>
</div>`;
        })
        .join("");
      ...
      // 显式预览按钮 → 模型详情
      list.querySelectorAll("[data-preview-path]").forEach((el) => {
