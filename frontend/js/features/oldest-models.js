...
export async function loadOldestModel(container, esc) {
  ...
  async function handleContainerClick(e) {
    ...
    if (btn) {
      ...
      if (!path) return;
      try {
        const { IsDirectory } = await import("../../wailsjs/go/main/App.js");
        const isDir = await IsDirectory(path);
        bus.emit("model:select", { path, isDir });
      } catch {
  ...
  async function render() {
    ...
    try {
          ...
          oldest4
            .map((e) => {
              ...
              return (
                ...
                "</span><span>🕒 " +
                ageDays +
                ' 天前</span></div><div class="actions"><button data-preview-path="' +
                esc(e.Path || e.Name || "") +
                '">👁 预览</button></div></div>'
              );
            })
        ...
        for (let i = 0; i < total; i++) {
          ...
          picks.push(
            '<div class="model-card" style="flex:1;min-width:140px;max-width:200px" data-path="' +
              ...
              '</div><div class="actions"><button data-preview-path="' +
              esc(p.Path || p.Name || "") +
              '">👁 预览</button></div></div>',
          );
        }
      ...
      container.innerHTML =
        '<div class="oldest-page" style="display:flex;flex-direction:column;gap:16px;padding:16px;overflow-y:auto;height:100%">' +
        '<div class="oldest-hero" style="display:flex;gap:16px;flex-wrap:wrap">' +
        '<div class="oldest-health" style="display:flex;align-items:center;gap:8px;background:var(--surf);border:1px solid var(--bd);border-radius:8px;padding:8px 14px;flex-shrink:0;align-self:flex-start">' +
        ...
        healthLabel +
        "</span></div>" +
        '<div class="oldest-pills" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">' +
        ...
        dupGroups +
        "</span></div>" +
        '<div class="oldest-panel oldest-ancient" style="flex:3;min-width:280px;background:var(--surf);border:1px solid var(--bd);border-radius:10px;padding:12px 14px">' +
        ...
        oldestHtml +
        "</div></div></div>" +
        '<div class="oldest-panel oldest-heatmap" style="background:var(--surf);border:1px solid var(--bd);border-radius:10px;padding:8px 12px">' +
        '<div style="font-size:var(--fs-sm);color:var(--muted);margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px">📅 月度活动</div>' +
        heatmapHtml +
        "</div>" +
        '<div class="oldest-panel oldest-picks" style="text-align:center;background:var(--surf);border:1px solid var(--bd);border-radius:10px;padding:16px">' +
