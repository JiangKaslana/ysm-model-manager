...
export function registerContextMenus() {
  bus.on(
    ...
    ({
      ...
      if (type === "dir") {
        const hasFolderIcon = Boolean(getFolderIcon(dir));
        bus.emit("menu:show", {
          x,
          y,
          items: [
            ...
            { divider: true },
            {
              ...
              icon: "▣",
              onClick: async () => {
                const dataURL = await pickImageDataURL({ maxSize: 320 });
                if (!dataURL) return;
                const ok = setFolderIcon(dir, dataURL);
                toast(
                  ok ? "文件夹封面已更新" : "文件夹封面保存失败",
                  2500,
                  ok ? "success" : "error",
                );
                refreshUI();
              ...
              ? [
                  {
                    ...
                    icon: "×",
                    onClick: () => {
                      const ok = removeFolderIcon(dir);
                      toast(
                        ok ? "文件夹封面已移除" : "文件夹封面移除失败",
                        2500,
                        ok ? "success" : "error",
                      );
                      refreshUI();
