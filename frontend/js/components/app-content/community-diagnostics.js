...
export async function startDedup(root, esc, rtype) {
  ...
  try {
    ...
    let html = `<div class="dedup-summary" style="padding:10px 12px;font-size:11px;color:var(--txt);border-bottom:1px solid var(--bd)">
发现 <strong>${totalGroups}</strong> 组重复文件（共 <strong>${totalDups}</strong> 个多余副本），每组选一个保留：
<span class="dedup-summary-note" style="display:block;font-size:9px;color:var(--muted);margin-top:2px">未选择的文件将移入回收站</span>
</div>`;

    let groupIndex = 0;
    for (const rtResult of allResults) {
      html += `<div class="dedup-type-head" style="display:flex;align-items:center;gap:4px;padding:6px 12px 2px;font-size:10px;font-weight:600;color:var(--txt)">
${rtResult.icon} ${rtResult.label}
<span class="dedup-type-rule" style="flex:1;border-bottom:1px solid var(--bd);margin-left:6px"></span>
<span class="dedup-type-count" style="font-size:9px;color:var(--muted);font-weight:400">${rtResult.groups.reduce((s, g) => s + g.files.length, 0)} 文件</span>
</div>`;
      ...
      for (const group of rtResult.groups) {
        ...
        html += `<div class="dedup-group" style="margin:4px 12px;border:1px solid var(--bd);border-radius:8px;overflow:hidden">
<div class="dedup-group-head" style="display:flex;align-items:center;gap:6px;padding:5px 8px;font-size:10px;font-weight:600;color:var(--txt);background:var(--surf);border-bottom:1px solid var(--bd)">
<span>📎 组 ${gi + 1}</span>
<span style="flex:1"></span>
<span class="dedup-group-meta" style="font-size:9px;color:var(--muted);font-weight:400">${files.length} 个文件 · ${totalSize} 字节</span>
</div>`;
        files.forEach((e, fi) => {
          ...
          html += `<label class="dedup-file-row${isDefault ? " is-recommended" : ""}" style="display:flex;align-items:center;gap:4px;padding:4px 8px;font-size:10px;cursor:pointer;transition:background .1s;background:${isDefault ? "var(--hover)" : "transparent"}">
<input class="dedup-radio" type="radio" name="dedup-keep-${gi}" value="${fi}"${checked} style="flex-shrink:0;accent-color:var(--accent)">
<span class="dedup-file-main" style="flex:1;overflow:hidden;min-width:0">
<span class="dedup-file-name" style="color:var(--txt);font-size:10px;cursor:pointer" title="点击查看详情: ${esc(e.path)}" data-path="${esc(e.path)}">${renderDisplayName(e.name)}</span>
<span class="dedup-file-path" style="display:block;font-size:8px;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">📁 ${esc(dir)}</span>
</span>
<span class="dedup-file-size" style="font-size:9px;color:var(--muted);flex-shrink:0;margin-right:4px">${(e.size / 1024).toFixed(0)}KB</span>
${dateStr ? '<span class="dedup-file-date" style="font-size:8px;color:var(--muted);flex-shrink:0">' + dateStr + "</span>" : ""}
${isDefault ? '<span class="dedup-badge" style="font-size:8px;padding:0 4px;border-radius:3px;background:#a6e3a122;color:#a6e3a1">推荐</span>' : ""}
</label>`;
        });
        html += `<label class="dedup-file-row dedup-keep-all" style="display:flex;align-items:center;gap:4px;padding:4px 8px;font-size:10px;cursor:pointer;transition:background .1s;border-top:1px solid var(--bd)">
<input class="dedup-radio" type="radio" name="dedup-keep-${gi}" value="-1" style="flex-shrink:0;accent-color:var(--accent)">
<span class="dedup-keep-text" style="color:var(--muted)">🔀 保留全部（不删除）</span>
</label>`;
        html += `</div>`;
      }
    }

    html += `<div class="dedup-actions" style="display:flex;gap:6px;padding:8px 12px;border-top:1px solid var(--bd)">
<button class="btn-base accent dedup-exec" id="diag-dedup-exec" style="flex:1;padding:7px 16px;border-radius:6px;border:none;background:var(--accent);color:#fff;cursor:pointer;font-size:11px">🗑️ 删除未选中的重复文件</button>
<button class="btn-base dedup-cancel" id="diag-dedup-cancel" style="padding:7px 16px;border-radius:6px;border:1px solid var(--bd);background:transparent;color:var(--muted);cursor:pointer;font-size:11px">取消</button>
</div>`;
    list.innerHTML = html;
