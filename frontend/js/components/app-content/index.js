      ...
      btn.addEventListener("click", async () => {
        ...
        if (!inited[tab] && tab !== ids[0]) {
          ...
          if (tab === "import") {
            ...
            container.innerHTML =
              '<div class="dedup-page" style="display:flex;flex-direction:column;height:100%">' +
              '<div class="dedup-toolbar" style="display:flex;align-items:center;gap:8px;padding:4px 12px;border-bottom:1px solid var(--bd)">' +
              '<span class="dedup-hint" style="flex:1;font-size:var(--fs-sm);color:var(--muted)">📌 按 SHA256 哈希分组，每组只保留一个，其余移入回收站</span>' +
              '<button class="btn-base accent dedup-start" id="dedup-start-btn">🔗 开始去重</button>' +
              "</div>" +
              '<div class="dedup-result-list" id="dedup-result-list" style="flex:1;overflow-y:auto;padding:8px 0"></div>' +
              "</div>";
            const doDedup = () => {
