...
export function repositoryHTML() {
  return (
    ...
    "</div>" +
    '<app-preview mode="model" style="width:260px;flex-shrink:0;border-left:1px solid var(--bd)"></app-preview>' +
    "</div>" +
    "</div>"
  );
}
...
<div style="display:flex;gap:12px">
  ...
  <div style="flex:1;background:var(--surf);border:1px solid var(--bd);border-radius:8px;padding:10px 14px">
    ...
    <select id="set-mirror" class="stg-select" style="width:100%;margin-bottom:6px">
      ...
      <option value="custom-proxy">🛰️ 自定义 HTTPS 代理</option>
      <option value="custom-github">🔧 自定义 GitHub 模板</option>
    </select>
    ...
    <div id="mirror-hint-custom-proxy" style="display:none;font-size:var(--fs-sm);color:var(--muted);padding:2px 0;line-height:1.5">通用 HTTPS 代理：下载时会变成「代理前缀/原始 URL」。<br>例如 https://all.hlmirror.com/https://github.com/...</div>
    <div id="mirror-hint-custom-github" style="display:none;font-size:var(--fs-sm);color:var(--muted);padding:2px 0;line-height:1.5">GitHub raw 模板：支持 {owner} {repo} {branch} {path} {url} 占位符。<br>适合兼容 jsDelivr、ghproxy 类服务。</div>
    <div class="mirror-custom-fields" id="mirror-custom-fields" style="display:none;margin-top:8px;gap:6px;flex-direction:column">
      <input id="set-mirror-base" class="stg-select mirror-input" placeholder="通用代理前缀，例如 https://all.hlmirror.com" style="width:100%">
      <input id="set-mirror-template" class="stg-select mirror-input" placeholder="GitHub 模板，例如 https://cdn.jsdelivr.net/gh/{owner}/{repo}@{branch}/{path}" style="width:100%">
      <button class="btn-base sm" id="set-mirror-save">💾 保存镜像配置</button>
    </div>
  </div>
</div>
...
<div class="settings-group cc-group" style="margin-bottom:12px">
  <div class="setting-row cc-row">
    <span class="label">🎨 主色调</span>
    <div class="cc-swatches" id="cc-presets"></div>
    <label class="cc-pick" title="自定义主色">
      <input type="color" id="set-accent" class="cc-color-input">
      <span class="cc-hex" id="cc-accent-hex"></span>
    </label>
    <button class="btn-base sm" id="cc-reset">↩️ 重置</button>
  </div>
  <div class="stg-hint">配色会实时应用到按钮、链接、选中态和高亮，且独立于主题保存。</div>
  <details class="cc-advanced">
    <summary>⚙️ 高级：背景与卡片颜色</summary>
    <div class="cc-adv-row">
      <label class="cc-adv-item"><span>背景</span><input type="color" id="set-bg"></label>
      <label class="cc-adv-item"><span>面板</span><input type="color" id="set-surf"></label>
      <label class="cc-adv-item"><span>卡片</span><input type="color" id="set-card"></label>
    </div>
    <div class="stg-hint">自定义背景色可能影响对比度，显示异常时重置即可恢复。</div>
  </details>
</div>

<div class="section-title stg-title stg-sub-title">🤖 AI 助手</div>

<div class="settings-group ai-group" style="margin-bottom:12px">
  <div class="ai-row">
    <label class="ai-field" style="flex:2">
      <span class="ai-label">接口地址（OpenAI 兼容）</span>
      <input type="text" id="set-ai-base" class="ai-input" placeholder="例如 https://api.deepseek.com/v1" autocomplete="off">
    </label>
    <label class="ai-field" style="flex:1">
      <span class="ai-label">模型名</span>
      <input type="text" id="set-ai-model" class="ai-input" placeholder="例如 deepseek-chat" autocomplete="off">
    </label>
  </div>
  <div class="ai-row">
    <label class="ai-field" style="flex:2">
      <span class="ai-label">API Key</span>
      <input type="password" id="set-ai-key" class="ai-input" placeholder="sk-..." autocomplete="off">
    </label>
    <button class="btn-base sm" id="set-ai-test" style="align-self:flex-end;height:30px">测试连接</button>
  </div>
  <div class="stg-hint">可接 DeepSeek、Kimi、OpenRouter、本地 Ollama 等 OpenAI 兼容服务。密钥仅保存在本地。</div>
</div>
...
export function downloadsHTML() {
  return `<div class="dl-page" style="flex:1;display:flex;flex-direction:column;overflow:hidden">
<div id="dl-form" style="margin:4px 12px;display:none;flex-direction:column;gap:4px">
  <div class="dl-form-head" style="font-size:11px;color:var(--muted);display:flex;align-items:center;gap:4px;flex-wrap:wrap">
    <span>导入仓库前，先重命名一下吧：</span>
    <label class="dl-check" style="display:flex;align-items:center;gap:2px;font-size:10px;color:var(--muted);cursor:pointer;white-space:nowrap">
  ...
  <div class="dl-fields" style="display:flex;gap:4px">
    <input class="dl-input" id="dl-author" placeholder="作者" style="width:90px;padding:4px 5px;border-radius:4px;border:1px solid var(--bd);background:var(--surf);color:var(--txt);font-size:11px">
    <input class="dl-input" id="dl-work" placeholder="作品品牌" style="width:90px;padding:4px 5px;border-radius:4px;border:1px solid var(--bd);background:var(--surf);color:var(--txt);font-size:11px">
    <input class="dl-input" id="dl-chara" placeholder="角色名" style="width:80px;padding:4px 5px;border-radius:4px;border:1px solid var(--bd);background:var(--surf);color:var(--txt);font-size:11px">
    <input class="dl-input" id="dl-variant" placeholder="变体" style="width:60px;padding:4px 5px;border-radius:4px;border:1px solid var(--bd);background:var(--surf);color:var(--txt);font-size:11px">
    <input class="dl-input" id="dl-date" placeholder="年月" style="width:64px;padding:4px 5px;border-radius:4px;border:1px solid var(--bd);background:var(--surf);color:var(--txt);font-size:11px">
  </div>
  ...
  <div class="dl-preview-row" style="display:flex;align-items:center;gap:8px;padding:4px 6px;border-radius:4px;background:var(--surf);overflow:hidden">
    <span class="dl-preview-label" style="color:var(--muted);font-size:9px;white-space:nowrap">最终命名</span>
    ...
    <button class="btn-base sm" id="dl-preview-model" style="white-space:nowrap">👁 预览</button>
    <button class="btn-base accent sm" id="dl-import" style="white-space:nowrap">📥 导入</button>
    <span class="dl-queue-chip" style="font-size:9px;color:var(--muted);white-space:nowrap">队列 <span id="dl-queue-count">0</span></span>
    <button class="btn-base sm" id="dl-cancel" style="white-space:nowrap">✕</button>
  </div>
</div>
<div class="dl-list-panel" style="margin:0 12px 4px;border-top:1px solid var(--bd);padding-top:4px">
  <div class="dl-list-head" style="display:flex;align-items:center;gap:6px;font-weight:600;color:var(--txt);padding:2px 0">
...
<div id="dl-drop" style="flex:1;margin:4px 12px;border:2px dashed var(--bd);border-radius:10px;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:4px;transition:all .2s;cursor:pointer;min-height:80px">
  <div class="dl-drop-icon" style="font-size:28px;opacity:.35">📥</div>
  <div class="dl-drop-title" style="font-size:11px;color:var(--muted)">拖拽模型文件 <b>.ysm .zip .7z .json .pmx .pmd .vrca .vrm .nbt .schematic</b> 或文件夹到此处，或点击选择文件</div>
  <div class="dl-drop-sub" style="display:flex;gap:8px;align-items:center">
    <span style="font-size:9px;color:var(--muted)">📁 拖入整个文件夹即可批量导入</span>
  </div>
  <div class="dl-drop-hint" style="font-size:10px;color:#bd93f9;margin-top:2px">💡 MMD/基岩版模型建议打包为 .zip 文件管理</div>
