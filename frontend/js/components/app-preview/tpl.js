...
<div class="dp-body" id="dp-body" style="display:none">
  ...
  <div class="dp-global-actions">
    <div class="dp-section-title">全局管理</div>
    <button class="btn-base accent sm" id="dp-btn-import-all">导入仓库模型</button>
  ...
  <div class="dp-log-filter" id="dp-log-filter" style="display:none;gap:4px;padding:2px 0">
    ...
    <button class="dp-log-fbtn" data-status="skipped">跳过</button>
  ...
  <div class="dp-log-footer" id="dp-log-footer" style="display:none">
    <button class="btn-base sm" id="dp-btn-clear-logs">清空</button>
  </div>
</div>
</div>`;
}
...
<div class="md-row"><span class="md-label">贴图</span><span class="md-value">${meta.textures || 0}</span></div>
...
<div class="md-row"><span class="md-label">面</span><span class="md-value">${(meta.faces || 0).toLocaleString()}</span></div>
</div>`;
}

export function statsCardHTML(model, modelPath, decodedBy) {
  const lower = modelPath.toLowerCase();
  const fmt = lower.endsWith(".ysm")
    ? ".ysm"
    : lower.endsWith(".zip")
      ? ".zip"
      : lower.endsWith(".7z")
        ...
        : lower.endsWith(".json")
          ? ".json"
          : "模型文件";
  const badge = decodedBy ? `<span class="ysm-badge">${esc(decodedBy)}</span>` : "";
  const variants = model.variants?.length || 0;
  const variantLine =
    variants > 1
      ? `<div class="ysm-card-row">形态 <span class="ysm-card-val">${(model.activeVariant || 0) + 1}</span> / ${variants}</div>`
      : "";
  ...
  if (tml && tml.length > 1) {
    texMapHtml = tml
      .map((m) => {
        const sizeStr = m.finalSize && m.finalSize !== "-" ? `，${esc(m.finalSize)}px` : "";
        const texName = m.texKey && m.texKey !== "-" ? m.texKey : `贴图 ${Number(m.texIdx || 0) + 1}`;
        return `<div class="ysm-card-row" style="font-size:9px;padding:1px 0" title="几何：${esc(m.file)}，贴图：${esc(texName)}${sizeStr}">${esc(m.file)} -> ${esc(texName)}</div>`;
      })
      .join("");
    texMapHtml = `<div class="ysm-card-section-label" style="margin-top:6px">贴图分配</div>${texMapHtml}`;
  }
...
<div class="ysm-card-section ysm-section-blue">
  ...
  <div class="ysm-card-row">
    骨骼 <span class="ysm-card-val">${model.boneCount || 0}</span>
    <span style="opacity:.55;margin:0 5px">/</span>
    方块 <span class="ysm-card-val">${model.cubeCount || 0}</span>
  </div>
  ${variantLine}
</div>
<div class="ysm-card-section ysm-section-green">
  <div class="ysm-card-section-label">贴图尺寸</div>
  <div class="ysm-card-row"><span class="ysm-card-val">${model.texWidth || "?"} x ${model.texHeight || "?"}</span> px</div>
  ${texMapHtml}
</div>
