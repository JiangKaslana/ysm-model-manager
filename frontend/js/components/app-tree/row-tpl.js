...
<span class="hover-actions">
  <span class="ha-btn ha-model-preview" data-path="${fp}" title="预览模型">👁</span>
...
export function folderRowHTML(
  ...
  const folderIcon = getFolderIcon(full);
  const iconHTML = folderIcon
    ? `<img class="folder-cover-icon" src="${attr(folderIcon)}" alt="">`
    : `<span class="folder-plain-icon">${fi}</span>`;
  const pad = indent != null ? ' style="padding-left:' + indent + 'px"' : "";
  return `<div class="fh${lk}" data-dir="${attr(full)}"${pad}>
<span class="ck${ckCls}" data-dir="${attr(full)}"></span>
<span class="ar${ac}">${ar}</span>
${iconHTML}
<span class="nm" style="color:${nc}">${dispName}</span></div>`;
}
