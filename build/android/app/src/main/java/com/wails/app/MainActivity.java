...
public class MainActivity extends AppCompatActivity {
    ...
    // `<input type="file">`（导入 tab 文件选择）经 WebChromeClient.onShowFileChooser
    // 走系统文档选择器；与 FILE_PICKER_REQUEST（Go SelectImportFile 桥）互不干扰。
    ...
    // 在途的 `<input type="file">` 选择回调（onShowFileChooser → onActivityResult 闭环；
    // 单例：新选择覆盖旧回调时先释放，防回调悬挂导致 input.files 永不返回）
    ...
    // onActivityResult 已处理授权结果 → 紧随其后的 onResume 跳过翻转检测（防双发）
    private boolean storageResultHandled;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // API 33+ 预测性返回：onBackPressed 已弃用，返回键经 OnBackInvokedDispatcher 接管
        // （manifest enableOnBackInvokedCallback="true"）；低版本仍走 onBackPressed() 覆写
        ...
        loadApplication();
        // 初始化存储授权基线：避免首帧 onResume 误发 storage:permissionGranted
    ...
    private void setupWebView() {
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            /**
             * 外链导航 → 系统浏览器（ACTION_VIEW），应用自身（wails.localhost）不拦。
             * 修复前：WebViewClient 未重写 shouldOverrideUrlLoading，点击外链被 WebView
             * 当作普通页面在当前 WebView 内加载，无任何"跳系统浏览器"事件。
             * API 21-23 走 String 重载，API 24+ 走 WebResourceRequest 重载（minSdk 21）。
             */
            @SuppressWarnings("deprecation")
            @Override
            public boolean shouldOverrideUrlLoading(WebView view, String url) {
                return openExternalOrLoad(view, Uri.parse(url));
            }

            @Override
            public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest request) {
                // iframe 内嵌浏览（工坊直连官网）的子帧导航放行给 WebView，不踢出到系统浏览器
                if (!request.isForMainFrame()) return false;
                return openExternalOrLoad(view, request.getUrl());
            }

            /** 主帧导航：非 wails.localhost 的 http/https 抛给系统浏览器；
             *  非 http/https scheme（javascript:/intent: 等）静默拦截防注入 WebView 上下文；
             *  打开失败时放行内嵌加载避免死链 */
            private boolean openExternalOrLoad(WebView view, Uri uri) {
                String scheme = uri.getScheme();
                String host = uri.getHost();
                if ("http".equals(scheme) || "https".equals(scheme)) {
                    if (!WAILS_HOST.equals(host)) {
                        try {
                            ...
                            Log.e(TAG, "openExternalOrLoad failed: " + uri, e);
                            return false;
                        }
                    }
                    return false; // wails.localhost 应用自身
                }
                if (scheme != null && !scheme.isEmpty()) {
                    Log.w(TAG, "blocked non-http(s) navigation: " + uri);
                    return true;
                }
                return false;
            }
        ...
        // `<input type="file">` 文件选择支持（导入 tab 的 dl-file-input）。
        // 默认 WebChromeClient 不处理文件选择 → Android WebView 点击静默无反应。
        // 走系统文档选择器（ACTION_GET_CONTENT），返回的 content:// URI 由系统
        // 临时授权，WebView 直接读（前端 FileReader→base64 流不变）。
        // 注：FileChooserParams 公共 API 无目录模式（MODE_OPEN_DIRECTORY 不存在），
        // 且 webkitdirectory 的 folderInput 在 Android 触屏不可达（Ctrl+点击），不处理。
        webView.setWebChromeClient(new WebChromeClient() {
            ...
            public boolean onShowFileChooser(WebView view, ValueCallback<Uri[]> filePathCallback,
                ...
                // 强制多选（导入 tab 支持批量；input 未声明 multiple 也允许）
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == MANAGE_STORAGE_REQUEST) {
            // Returned from "All files access" Settings screen. 授权结果在
            // onActivityResult 发一次（真实变化），置标志让紧随其后的 onResume
            // 跳过翻转检测——两处都发会重复触发 storage:permissionGranted
            // → JS 重复全库重扫。
            if (hasManageStoragePermission() && bridge != null) {
                bridge.emitEvent("storage:permissionGranted", "{}");
            }
            storageResultHandled = true;
            return;
        }
        if (requestCode == WEBVIEW_FILE_CHOOSER_REQUEST) {
            // `<input type="file">` 选择结果 → 回调 WebView（取消时 results 为 null）
            if (pendingWebFileChooser != null) {
                Uri[] results = null;
                ...
                pendingWebFileChooser.onReceiveValue(results);
                pendingWebFileChooser = null;
            }
            return;
        }
    ...
    private void emitBattery(@Nullable Intent batteryStatus) {
        try {
            ...
            if (bridge != null) bridge.emitEvent("android:BatteryChanged", o.toString());
        } catch (Exception e) {
    ...
    private void emitNetwork(@Nullable Network network) {
        try {
            ...
            if (bridge != null) bridge.emitEvent("android:NetworkChanged", o.toString());
        } catch (Exception e) {
    ...
    private void emitLock(boolean locked) {
        ...
        if (bridge != null) {
            bridge.emitEvent(locked ? "android:ScreenLocked" : "android:ScreenUnlocked", "{}");
        }
    }
    ...
    private void emitTheme() {
        try {
            ...
            if (bridge != null) bridge.emitEvent("android:ThemeChanged", o.toString());
        } catch (Exception ignored) {
        }
    }
    ...
    protected void onResume() {
        ...
        boolean handled = storageResultHandled;
        storageResultHandled = false;
        if (!handled && nowGranted && !lastStorageGranted) {
            if (bridge != null) {
                bridge.emitEvent("storage:permissionGranted", "{}");
            }
        }
    ...
    // ── 外部存储授权（模型库 MANAGE_EXTERNAL_STORAGE，ADR-046 P2）──
     ...
     *  unable to read the user-selected repo path. */
    public void requestStoragePermission() {
        ...
        runOnUiThread(() -> {
            new AlertDialog.Builder(this)
                .setTitle("需要文件访问权限")
                .setMessage("YSM 模型管理器需要读取模型仓库目录中的模型与资源文件。\n\n" +
                ...
                .setNegativeButton("稍后", (DialogInterface d, int which) -> {
                    Toast.makeText(this, "未授权，模型库将无法读取仓库目录", Toast.LENGTH_LONG).show();
                    d.dismiss();
                })
                .setCancelable(false)
                .show();
    ...
    protected void onDestroy() {
        ...
        unregisterSystemEventReceivers();
        if (pendingWebFileChooser != null) {
            // WebView 要求销毁时回调 null，否则 `<input type="file">` 的
            // files 状态悬挂（低内存回收/语言切换重建 Activity 也会走此路径）
            pendingWebFileChooser.onReceiveValue(null);
            pendingWebFileChooser = null;
        }
        if (bridge != null) {
    ...
    /**
     * 返回键统一处理（WebView 可后退先 goBack；否则双击退出，首按通知 JS）。
     * API 33+ 由 OnBackInvokedDispatcher 调用；低版本由 onBackPressed() 覆写调用。
     */
    ...
    @Override
    @SuppressWarnings("deprecation") // API 33+ 已由 OnBackInvokedDispatcher 接管，本覆写仅低版本生效
