...
> ⚠️ mirror 对齐标注（2026-08-11 子代理审计）：上表全字段归零报告日期早于
> mirror/inflate 补齐（2026-08-09）。`csharp-builder.mjs` **无 mirror 处理**（C#
> ThreeJsPayloadBuilder 无 mirror 分支）；Go 侧 `spec.go:485-490` 对 `c.Mirror` 做
> UV 水平翻转，对齐的是 **Java GeoQuad 的 mirror 分支**而非 C#。当前 YSMParser
> 解码器（YSMParserV3.cpp）序列化 cube 时不输出 mirror 字段（已烘进负 uv_size），
> 故解码产物路径两测均不触发——含 mirror 的原始 JSON 跑 compare.mjs 会在 UV 上
> 分叉，属预期差异，勿误判为 Go 回归。
