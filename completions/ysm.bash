...
_ysm_complete() {
  ...
  if [ "$COMP_CWORD" -eq 1 ]; then
    COMPREPLY=( $(compgen -W "analyze analyze-mmd avatar benchmark cache-clear cache-diag cache-status cache-verify concurrent-bench config config-show copy creator dedup download export file-bench gui-flow health-report hub install instance link-mode list move perf-log perf-snapshot recycle rename repo-audit resource-scan resource-types scan scan-dir search single-bench tags toggle verify workshop" -- "$cur") )
    return
  fi
