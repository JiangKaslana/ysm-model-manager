...
func TestDetectPCLIsolatedInstance(t *testing.T) {
	...
	if len(got) != 1 { t.Fatalf("instances = %d, want 1: %#v", len(got), got) }
	inst := got[0]
	if inst.Launcher != "PCL" || inst.GameVersion != "1.20.1" { t.Fatalf("unexpected identity: %#v", inst) }
	if inst.GameDir != versionDir { t.Errorf("GameDir = %q, want %q", inst.GameDir, versionDir) }
	wantCustom := filepath.Join(versionDir, "config", "yes_steve_model", "custom")
	if inst.CustomDir != wantCustom { t.Errorf("CustomDir = %q, want %q", inst.CustomDir, wantCustom) }
}
...
func TestDetectPCLSharedInstance(t *testing.T) {
	...
	got, err := Detect(root)
	if err != nil { t.Fatal(err) }
	if len(got) != 1 || got[0].Launcher != "PCL" { t.Fatalf("unexpected result: %#v", got) }
	if got[0].GameDir != gameRoot { t.Errorf("GameDir = %q, want shared root %q", got[0].GameDir, gameRoot) }
}
...
func TestDetectRejectsPlainMinecraftDirectory(t *testing.T) {
	...
	got, err := Detect(root)
	if err != nil { t.Fatal(err) }
	if len(got) != 0 { t.Fatalf("plain Minecraft directory must not be reported as HMCL/PCL: %#v", got) }
}
...
func TestDetectHMCLConfiguredDirectoryAndRunningDirectory(t *testing.T) {
	...
	if len(got) != 1 { t.Fatalf("instances = %d, want 1: %#v", len(got), got) }
	inst := got[0]
	if inst.Launcher != "HMCL" || inst.GameVersion != "1.21.1" || inst.GameDir != runDir { t.Fatalf("unexpected result: %#v", inst) }
}
...
func TestDetectHMCLModpackUsesVersionDirectory(t *testing.T) {
	...
	got, err := Detect(root)
	if err != nil { t.Fatal(err) }
	if len(got) != 1 || got[0].GameDir != versionDir { t.Fatalf("unexpected result: %#v", got) }
}

func TestDetectRejectsFile(t *testing.T) {
	file := filepath.Join(t.TempDir(), "PCL.exe")
	writeFile(t, file, "")
	if _, err := Detect(file); err == nil { t.Fatal("Detect(file) should fail") }
}
