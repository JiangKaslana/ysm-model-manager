...
func WriteFileAtomic(destPath string, data []byte) error {
	if err := fsutil.WriteFileAtomic(destPath, data); err != nil {
		if errors.Is(err, fsutil.ErrTempCreateFailed) {
			return types.AppError{Code: "MKDIR_FAILED", Operation: "导入模型", TargetPath: filepath.Dir(destPath), Reason: "无法创建临时文件: " + err.Error(), Suggestion: "请检查磁盘权限或空间"}.WithCause(err)
		}
		return types.AppError{Code: "WRITE_FAILED", Operation: "导入模型", TargetPath: destPath, Reason: "写入失败: " + err.Error(), Suggestion: "请检查磁盘权限或空间"}.WithCause(err)
	}
	return nil
}
