...
func ParseFromZip(data []byte, size int64) (*types.BedrockModel, [][]byte, []string) {
	...
	for _, f := range reader.File {
		...
		if strings.HasSuffix(low, "ysm.json") && !f.FileInfo().IsDir() {
			...
			if err := json.Unmarshal(buf, &ysm); err != nil {
				...
				modelOrder = parsePlayerMainModelRefs(ysm.Files.Player.Model)
			}
			break
		}
	}
	...
	for _, gf := range geoFiles {
		if len(modelOrder) > 0 && pathOrderScore(gf.name, modelOrder) >= 1000 {
			continue
		}
		models := ParseBedrockGeometryVariants(gf.data, gf.name)
		for _, m := range models {
			if m.BoneCount > 0 {
				if texIndex := pathOrderScore(gf.name, modelOrder); texIndex < 1000 {
					m.TexIndex = texIndex
				}
	...
	geo = SelectPrimaryModel(variants, modelOrder)
...
func ParseFrom7z(data []byte, size int64) (*types.BedrockModel, [][]byte) {
	...
	for _, f := range reader.File {
		...
		if strings.HasSuffix(low, ".json") && !strings.Contains(low, "ysm.json") && !f.FileInfo().IsDir() {
			...
			buf, _ := io.ReadAll(io.LimitReader(rc, maxExtractSize))
			rc.Close()
			models := ParseBedrockGeometryVariants(buf, f.Name)
			for _, m := range models {
				if m.BoneCount > 0 {
					variants = append(variants, m)
				}
	...
	geo = SelectPrimaryModel(variants, nil)
	return geo, pngs
}

func parsePlayerMainModelRefs(raw json.RawMessage) []string {
	if len(raw) == 0 {
		return nil
	}
	...
	default:
		...
		return add(nil, strings.Trim(trimmed, `"`))
	}
}
