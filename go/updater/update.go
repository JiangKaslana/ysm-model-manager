...
func Check(current string) (*UpdateInfo, error) {
	return CheckWithURLRewrite(current, nil)
}

// CheckWithURLRewrite 检查更新，并允许调用方为 GitHub API / SHA256SUMS 请求改写 URL。
func CheckWithURLRewrite(current string, rewrite func(string) string) (*UpdateInfo, error) {
	cur := normalize(current)
	if rewrite == nil {
		rewrite = func(url string) string { return url }
	}
	...
	req, err := http.NewRequest("GET", rewrite(api), nil)
	if err != nil {
		return nil, err
	}
	...
	if latestSHASumsURL != "" {
		expectedHash = fetchExpectedHash(rewrite(latestSHASumsURL), assetPattern())
	}
