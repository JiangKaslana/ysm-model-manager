...
func TestIsYsmFile_ZipWithYsmJson(t *testing.T) {
	path := testutil.WriteZipFile(t, "pack.zip", map[string]string{"sub/ysm.json": `{"spec":2}`})
	if !isYsmFile(path) {
...
func TestIsYsmFile_ZipWithModels(t *testing.T) {
	path := testutil.WriteZipFile(t, "pack.zip", map[string]string{"models/head.geo.json": `{}`})
	if !isYsmFile(path) {
...
func TestDetectResourceType_YsmDetector(t *testing.T) {
	...
	// .zip 含 ysm.json → 匹配
	zipPath := testutil.WriteZipFile(t, "pack.zip", map[string]string{"ysm.json": `{"spec":2}`})
