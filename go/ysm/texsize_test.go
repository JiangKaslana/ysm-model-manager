...
func TestReadTexSizeFromFile_Zip(t *testing.T) {
	...
	path := testutil.WriteZipFile(t, "mod.jar", map[string]string{"model.geo.json": geomJSON})
...
func TestReadTexSizeFromFile_ZipNoGeometry(t *testing.T) {
	...
	path := testutil.WriteZipFile(t, "mod.jar", map[string]string{"data.txt": "hello"})
...
func TestReadTexFromZip_Valid(t *testing.T) {
	...
	path := testutil.WriteZipFile(t, "mod.jar", map[string]string{"geo/model.geo.json": geomJSON})
...
func TestReadTexFromZip_NonGeoJSON(t *testing.T) {
	...
	path := testutil.WriteZipFile(t, "mod.jar", map[string]string{"data.json": `{"key":"value"}`})
	w, h := readTexFromZip(path)
	if w != 0 || h != 0 {
...
func TestScanModelTexSizes_WithEntries(t *testing.T) {
	// 构造含 geometry JSON 的 zip
	geomJSON := `{"minecraft:geometry":[{"description":{"texture_width":64,"texture_height":32}}]}`
	zipPath := testutil.WriteZipFile(t, "mod.jar", map[string]string{"geo.json": geomJSON})
