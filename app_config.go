...
func (a *App) SaveAppConfig(repoRoot, rpRoot, mcRoot, linkMode, theme string) error {
	...
	cfg := types.AppConfig{
		...
		MirrorCustomBase: oldCfg.MirrorCustomBase,
		MirrorCustomURL:  oldCfg.MirrorCustomURL,
...
func (a *App) SetDownloadMirrorConfig(mirror, customBase, customURL string) error {
	cfg := a.LoadAppConfig()
	cfg.Mirror = mirror
	cfg.MirrorCustomBase = customBase
	cfg.MirrorCustomURL = customURL
	return a.saveConfig(cfg)
}

func (a *App) rewriteDownloadURL(url string) string {
	return rewriteMirrorURL(a.LoadAppConfig(), url)
}

func rewriteMirrorURL(cfg types.AppConfig, url string) string {
	switch cfg.Mirror {
	case "custom-proxy":
		base := strings.TrimRight(strings.TrimSpace(cfg.MirrorCustomBase), "/")
		if base == "" || !strings.HasPrefix(url, "https://") {
			return url
		}
		...
		return base + "/" + url
	case "custom-github":
		if custom := rewriteGitHubTemplate(url, cfg.MirrorCustomURL); custom != "" {
			return custom
		}
	}
	return url
...
func rewriteGitHubTemplate(rawURL, template string) string {
	template = strings.TrimSpace(template)
	if template == "" {
		return ""
	}
	info, ok := parseGitHubRawURL(rawURL)
	if !ok {
		if strings.Contains(template, "{url}") {
			return strings.ReplaceAll(template, "{url}", rawURL)
		}
		return ""
	}
...
func parseGitHubRawURL(rawURL string) (githubRawInfo, bool) {
	const prefix = "https://raw.githubusercontent.com/"
	if !strings.HasPrefix(rawURL, prefix) {
		return githubRawInfo{}, false
	}
	parts := strings.SplitN(rawURL[len(prefix):], "/", 4)
	if len(parts) < 4 {
		return githubRawInfo{}, false
	}
	return githubRawInfo{
		Owner:  parts[0],
		Repo:   parts[1],
		Branch: parts[2],
		Path:   parts[3],
...
func (a *App) CheckUpdate() (*updater.UpdateInfo, error) {
	return updater.CheckWithURLRewrite(version.Version, a.rewriteDownloadURL)
}

func (a *App) DownloadUpdate(url string, expectedHash string) (string, error) {
	return updater.Download(a.rewriteDownloadURL(url), expectedHash)
}
...
func (a *App) DoUpdate(url string, expectedHash string) string {
	zipPath, err := updater.Download(a.rewriteDownloadURL(url), expectedHash)
	if err != nil {
