 ...
 *   node scripts/gen-cli-completion.mjs --check    # 只对比不写盘（doctor/pre-push 守护）
 *   node scripts/gen-cli-completion.mjs --json     # JSON 摘要（候选清单，测试/子代理消费）
 *
 * 退出码：--check 过期 → 1；否则 0。
 */
...
# 生成：node scripts/gen-cli-completion.mjs（顶层命令 ${count} 个）
...
if (CHECK) {
  for (const [file, content] of Object.entries(outputs)) {
    ...
    if (onDisk !== content) {
      rc = 1;
      if (!JSON_OUT) console.error(`[gen-cli-completion] completions/${file} 过期，运行 \`node scripts/gen-cli-completion.mjs\` 刷新。`);
    }
  }
