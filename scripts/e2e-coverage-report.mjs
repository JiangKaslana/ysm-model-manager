#!/usr/bin/env node
/**
 * e2e-coverage-report.mjs — 端到端广度报告（ADR-035 G-4）。
 *
 ...
 *
 * 用法：
 *   node scripts/e2e-coverage-report.mjs                    # 广度报告（默认 top 20）
 *   node scripts/e2e-coverage-report.mjs --input <path>     # 指定 V8 coverage JSON
 *   node scripts/e2e-coverage-report.mjs --all              # 列出全部文件
 *   node scripts/e2e-coverage-report.mjs --json             # 结构化输出（子代理消费）
 *
 * 边界（ADR-035 G-4 原文）：
