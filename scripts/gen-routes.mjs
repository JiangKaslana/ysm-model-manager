#!/usr/bin/env node
/**
 * gen-routes.mjs — AI 知识库路由表自动生成器（适配自 MikuMikuAR，ADR-114 §被补充）。
 *
 ...
 *   - 共享 ADR 关联阅读在 YSM 暂不产出（多数卡暂无 adr: 字段，见 gen-knowledge-adr.mjs）
...
function renderRoutes(cards) {
  ...
  out.push('修改后运行最小相关测试和 node scripts/doctor.mjs --docs。');
  out.push('```');
  out.push('');
  out.push('## 维护规则');
  out.push('');
  out.push('- 本文件自动生成，**请勿手改**；重跑 `node scripts/gen-routes.mjs` 重新生成。');
