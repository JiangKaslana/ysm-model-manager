#!/usr/bin/env node
/**
 * rollback-impact.mjs — revert 影响面分析（audit-split 的逆向镜像）。
 *
 ...
 * 保持"保留/搬家/真删"口径一致（详见 audit-split.mjs:140-171 注释）。
 *
 * 依赖：scripts/_lib/{git-ref,source-graph,scan-files}.mjs（零外部依赖）。
 *
 * 用法：
 *   node scripts/rollback-impact.mjs <commit>                    # 默认扫描 go/ + frontend/src/
 *   node scripts/rollback-impact.mjs <commit> --scope frontend/  # 限定扫描范围
 *   node scripts/rollback-impact.mjs <commit> --json             # JSON（供子代理/CI 消费）
 *   node scripts/rollback-impact.mjs <commit> --quiet            # 无调用方时静默（不报"无影响"）
 *
...
const result = analyze(commitArg);
