 ...
 *
 * 典型用法：
 *   node scripts/trace-analyze.mjs <trace.json>                    # 默认文本报告
 *   node scripts/trace-analyze.mjs <trace.json> --json             # JSON（供子代理/CI 消费）
 *   node scripts/trace-analyze.mjs <trace.json> --top 50           # 加长 Top 列表
 *   node scripts/trace-analyze.mjs <trace.json> --pid 39456 --tid 13360  # 指定线程明细
 *   node scripts/trace-analyze.mjs <a.json> <b.json>               # 双 trace 对比模式
 *
...
for (let i = 0; i < ARGS.length; i++) {
  ...
  if (a === '--json') { jsonOut = true; }
    ...
    console.log(`用法：node scripts/trace-analyze.mjs <trace.json> [trace2.json] [选项]
...
if (files.length === 0) {
  console.error('错误：缺少 trace 文件路径。用法：node scripts/trace-analyze.mjs <trace.json>');
  process.exit(2);
}
