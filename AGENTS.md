...
- **生成物**（`docs/` 下 index / funcmap / project-map / cli-commands、`frontend/public/locales/*.json`、`completions/` 等，由 `.githooks/pre-commit` 的 `GEN_CMDS` 产出）= 全体输入的纯函数。**不承担提交归属、不按归属裁剪**：改卡后由 pre-commit 自动 gen+stage，交就交当前全量态。
...
规模可达多主模型 × 各自数个子代理（10+ AI）。原则：**划范围 → 放手改 → 一眼抽查 → 自主汇总**；主模型是协作者不是监工，全程启用编辑模式。
