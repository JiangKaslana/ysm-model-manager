...
### 验证（diff 抽检 + 所有权验证）
- 子代理改完 → **先跑相关测试验证编译，再提交自己的改动清单**：哪些文件改了、哪些没改（只列文件名即可）
- 主模型验证：
  1. **所有权验证**：对比实际 diff 与锁文件范围，超出范围就是越权——立刻打回，禁止进入汇总
  2. **diff 抽检**：看 `git diff` 摘要（改了哪些文件、多少行、关键变更）
- 测试通过 + 无越权 + diff 合理 → 采纳，不逐行审。
...
### 互修规则（阻断级卡顿）
- 遇到阻断级卡顿，多个 AI 可自然互修 → 互修无需抢所有权，改完留工作区即可
- 每个 AI 不必反复查 git log 验证是否被覆盖，信任主模型提交前一次性 diff 汇总
- 互修改动不单独提交，保留现场等主模型收尾
