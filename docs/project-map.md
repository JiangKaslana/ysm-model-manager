...
| `rustbridge/` | Windows Rust 扫描 DLL 的嵌入、校验、加载与窄 ABI 适配层 〔源码 5: bridge_android.go bridge_windows.go doc.go embedded_windows.go types_windows.go · 子目录 1: bin/〕 |
| `scanner/` | 模型扫描 + 作者提取 + 仓库索引（ADR-003 P2 Logic Sinking） 〔源码 4: rust_backend_android.go rust_backend_stub.go rust_backend_windows.go scanner.go · 测试 7〕 |
