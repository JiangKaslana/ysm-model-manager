...
| `geometry/` | Bedrock Geometry JSON 解析（ZIP/7z 提取，防炸弹限制） 〔源码 3: archive.go parse.go ysm_parser.go · 测试 15 · 子目录 1: testdata/〕 |
...
| `utils/` | 工具函数（display / fmt / dom / icon / summarize / model3d） 〔源码 6: array.ts gh-links.ts main-thread-watch.ts module-loader.ts safe-error-msg.ts types-re-export.ts · 测试 3 · 子目录 9: 3d/ animation/ cache/ core/ debug/ dom/ format/ icon/ resource/〕 |
