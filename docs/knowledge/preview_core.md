...
- **ADR-076 v3 声明式根菜单（Phase 1+2 已落地，Phase 3 待立项）**：
  ...
  - **公共映射 previewItemToNode（方案 A 第 3 步，commit 86208061）**：`PreviewMenuItemDef`→`PreviewMenuNode` 单向映射集中一处（模块级导出），roleDetailView 两处 24 行重复样板收敛为 `children.map(previewItemToNode)`；结构字段数据化、内容经 renderCustom 逃生舱（动态数据面板按 MikuMikuAR 官方定位保留）。测试 +2（字段逐一对齐 + closePopup 兜底）。
  ...
  - **camera self 模式守卫（commit 7cb10aec）**：`PreviewMenuItemDef` / `PreviewMenuNode` 加 `hideInSelfMode?: boolean`（self 模式隐藏），camera 项置 true——self 模式相机由适配器自驱（如 MMD 相机动画每帧覆盖），camBridge 控件（旋转/速度/重置）操作核心 controls 被覆盖呈现「无效空面板」，隐藏最诚实。renderDock 过滤链加 `.filter(d => !(d.hideInSelfMode && ctx.selfMode))`；previewItemToNode 透传。self 模式 scene 组仍显（lighting/shadow/postproc 保留）。
...
- **方案 A 改造完成（2026-08-25 复核）**：迁移约 **100%**——所有面板（core 6 + 适配器注入项）均经声明式 `PreviewMenuNode` 渲染。`setAdapterItems` 接受 `PreviewMenuNode[]` 并内部转换为 `PreviewMenuItemDef` 兼容渲染链。`renderPanel` 统一走 `schemaBuilders` 或 `previewItemToNode` → `renderSchemaContent`。`PreviewMenuNode` 类型新增 `danger`/`legacyTestId` 兼容旧字段。所有适配器（ysm/mmd/vrm/litematic）返回 `PreviewMenuNode[]` 而非 `PreviewMenuItemDef[]`。`nodeToDef` 逆向映射提供过渡。剩余：
  1. ✅ **新菜单项硬截止**：已完成。
  2. ✅ **全量迁面板**：已完成。
  3. 🔄 **删逃生舱**：`PreviewMenuItemDef.render`/`run` 字段仍保留（供内部 `nodeToDef` 转换），`fillers` 仅剩 `environment`/`roles` 两项。待 `fillers` 和 `scene-registry.menuItems` 全部迁为 `PreviewMenuNode` 后可删除。
