...
- 异步事件 handler 全包 try/catch：导入按钮、列表点击（`ToggleResourcePack` / `_showDetail` 抛错转 `_toast("error","操作失败")`）与删除按钮均自带 try/catch；async handler 的 rejection 无处可传，不接住就是按钮静默失效
- `registerResourceManagerGlobal(unsubs)`（模块级导出）：监听 `config:updated` 清空 `STORE._config` 缓存（F8 修复：组件生产零实例，不再空转刷新实例；下次 `_init` 自动重新拉取）
...
- 监听 bus：`config:updated`（模块级订阅：清空类型配置缓存 `STORE._config`，下次 `_init` 自动重新拉取）
...
- 由 `app-content` 仓库页标签（resourcepacks / shaderpacks）与整合包内子页实例化；`config:updated` 由设置页配置变更时派发（见知识卡 `app_content`）
- Go binding 位于 `internal/app/resource_bindings.go` / `app_scan.go`（`ScanModelEntries`）
- 删除走 Go 回收站策略（`go/recycle`），前端以 `modalConfirm`（`utils/dom/dialogs/modal.ts`）二次确认防呆，不用原生 `confirm`

## 不变量

- 不在前端手写资源类型/扩展名/安装目录逻辑，一切以 `resource_types.json` 条目为准（治理红线：注册表优先）
- 模块级 `STORE._config` 缓存仅在 `config:updated` 或强制刷新时失效
