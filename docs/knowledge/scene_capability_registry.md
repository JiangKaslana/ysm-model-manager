...
invariant_anchors:
  ...
  - frontend/src/utils/3d/adapters/preview-menu.ts|renderEnvLevel
---
...
- **preview-menu / preview-menu-env**：环境面板由 `preview-menu.ts` 经 `renderEnvLevel`（`preview-menu-env.ts` 实现）只收 **ENV_IDS 白名单**（sky/ground/environment/fog/reflector，`getAll().filter(ENV_IDS.has)`），`fillLighting` 查 `light`，阴影面板查 `shadow`——同一能力控件**不会双面板重复**
