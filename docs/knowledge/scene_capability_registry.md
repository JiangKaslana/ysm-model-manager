...
invariant_anchors:
  ...
  - frontend/src/utils/3d/adapters/preview-menu.ts|fillEnvironment
---
...
- **preview-menu**：`fillEnvironment` 只收 **ENV_IDS 白名单**（sky/ground/environment/fog/reflector，`getAll().filter(ENV_IDS.has)`），`fillLighting` 查 `light`，阴影面板查 `shadow`——同一能力控件**不会双面板重复**
