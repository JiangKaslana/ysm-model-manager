...
- `content-layout.ts` — 基础层：`::host` 变量（`--tag-*` / `--badge-*` / `--hm-*` / `--sidebar-w` / `--diag-left-w` / `--touch-min`）+ 通用 keyframes（`pageIn` / `ring-spin` / `card-in` / `detail-in` / `fade-in` / `dl-slide-up`）+ 骨架（`.page` / `.stat-card` / `.placeholder-box`）+ 通用卡片系统（`.model-card` / `.model-card-sm` / `.rec-card` / `.health-ring`）+ 旧按钮兼容层（`.btn` 裸类：仅设置页 3 处遗留，待迁移 `.btn-base`；`.hdr-btn` 已删，死代码）+ 工坊通用按钮类（`.ws-*`：从 content-creator.ts 归位，跨 creator/gh 复用）
...
- `content-diag.ts` — 诊断页（`.diag-*` / `.perf-*` / `.log-row` / `.conflict-row` / `.scan-*` 动画）+ 设置页残留（`.settings-group` / `#set-advanced-panel`）+ GitHub 工坊（`.gh-*` 全族：仓库列表 / 模型行 / 二级菜单 / 下载队列 / 错误页）
