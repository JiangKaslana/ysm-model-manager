...
| 标识 | 名称 | tier | 性能 | 关键词 |
|------|------|------|------|--------|
| 🏗 animation-system | 动画系统 animation | architecture | cpu-bound | 动画, 骨骼动画, 关键帧, 动画播放, Molang, 数字滚动, stagger 入场, 关闭动画 |
...
- **utils-dom**（DOM 工具 dom）：HTML 转义、搜索高亮与全局 toast 时长语义常量。`esc()` 是全前端 HTML 转义的统一入口，也是治理红线指定的转义函数；`toast-ms.ts` 是全应用 toast 时长的单一事实源（6 档语义常量，消费方禁止内联魔法…
...
| 标签 | 含义 | 卡片 |
|------|------|------|
| cpu-bound | CPU 密集（解析/编译/解算/编码） | animation-system, model-stats, optimization_log, ysm-wasm |
| io-bound | IO 密集（批量读写/RPC/网络） | backend-idb, go-download, go-scanner, rustbridge |
| concurrent | 多核并行（goroutine 池/Worker 池/pthread/Promise 竞速） | go-scanner, model-stats, optimization_log, rustbridge |
| single-thread | 单线程顺序执行（顺序流水线/串行队列） | go-download, ysm-wasm |
| gpu-bound | GPU/显存敏感（纹理/3D 渲染） | optimization_log |
| memory-heavy | 内存/显存大户（大缓冲/长驻缓存） | optimization_log |
