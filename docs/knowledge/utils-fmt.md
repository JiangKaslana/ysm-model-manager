...
- `formatBytes(b: number): string` — 字节数 → `"N B"` / `"N.N KB"` / `"N.N MB"` / `"N.N GB"`（KB/MB/GB 阈值 1024 / 1048576 / 1073741824）；非法值（null/undefined/NaN/**±Infinity**）返回空串（P2 修复：原 `!b && b !== 0` 守卫挡不住 truthy 的 ±Infinity → 输出 "Infinity GB"）
