...
- `parseModelName(raw: string): ParsedModelName` — 解析文件名为 `{ raw, isBanned, author, work, chara, character, date, ext }`；`[作者]`/`[[作者]]`、`【作品】`/`《作品》` 三组括号经 `BRACKET_STYLES` 注册表统一（parse 与 render 共用 `bracketRe`，新增风格只改一处）；`YYYY[-_.]MM` 日期；`.ban` 后缀标记封禁文件
...
- 治理红线：**所有 UI 文件名展示必须走 renderDisplayName**（AGENTS.md §3.3），禁止把原始文件名直接拼进 innerHTML。**例外已记录**（P3 观察）：app-tree 搜索态（render.ts 搜索分支）走 `hl()` 而非 renderDisplayName——`hasSearch ? hl(e.name, search) : renderDisplayName(e.name)`，搜索时丢失 tag-work/tag-date 着色、§ 着色与 `.ban` 原文语义（hl 会 esc，XSS 安全）
