...
- **`parseModelFromEntries` 已拆为声明序管道**（2026-08-25 refactor，行为不变、逐段提交）：~407 行全仓最胖函数坍缩为装配器，依次调用 `collectMaidManifest`（L0 清单）→ `parseYsmArchive`（ysm.json）→ `deriveModelTexOrder`（model/tex 声明序派生）→ `resolveL0`（L0 先行判定，2026-08-26 调序）→ 命中走 `collectAnimEntriesOnly`（只收动画字符串，geo/png 不物化——清单生效时全量收集纯属白付 IO+内存）/ 未命中走 `collectMergedFiles`（③ 合并版收集：排 arm）→ `filterArmModels` → `sortByModelOrder` → `mergeGeoFiles`（⑥ texIdxMap+bones 合并）→ `sortByTexOrder` → `buildSubModels`（⑦）。各函数**纯搬移不改行为**，导出签名（ParseFromZip 四值返回链）未动；`collectArchiveFiles` 的 7 个位置返回值收敛为私有 `collectedArchive` 结构体（**放 archive.go 包内，勿升 `go/types/`**——那是跨包复用专用）。两条隐式时序约束写进函数签名/注释，勿因拆函数松动：① `sortByTexOrder` 必须在 `buildSubModels` 之前（orderMap 由前者返回、L0 TexSlot 按排序后槽位换算）；② `mergeGeoFiles`/`sortByModelOrder` 先于 `sortByTexOrder`（texIdxMap 消费 modelOrder）。三条红线不得顺手统一：`buildSubModels` 的 L0 覆盖判定只看 `len(maidManifest)>0`、不看 `l0.hit`，与 geoFiles 等覆盖判定不对称（现状事实，勿对齐）；`collectMergedFiles`（模型版，保留扩展名）与 `collectArchiveFiles`（清单版，去扩展名）口径分叉不可合并（`ysm_parser.go` 头注释钉死）；`collectMergedFiles` 里 `IsArmModelName` 检查在 Open+Read 之后（保持原序，勿优化成先判断再读）。行为锁靠 `archive_parse_behavior_lock_test.go` + `archive_merge_behavior_lock_test.go` 守住
- **⚠️ 测试文件名 GOARCH 后缀陷阱（2026-08-26 破案）**：`archive_arm_test.go` 曾被 Go 工具链**静默排除**（`.IgnoredTestGoFiles`）——文件名 `_arm` 段是合法 GOARCH 平台后缀，amd64 上该文件从不参与编译，三个测试自诞生起从未运行过（二分排查内容/编码/重名全无果后 `GOARCH=arm go list` 对照证实）。已改名 `archive_arm_models_test.go` 复活。**铁律：Go 文件名 `_test.go` 前的末段不得撞 GOOS/GOARCH 保留字（linux/windows/darwin/arm/amd64/386/arm64/wasm…）**；语义性后缀拼长（如 arm→arm_models）规避
- **2026-08-26 审查修复（确定性/口径统一）**：① `resolveComponentTexName` 前缀兜底多命中（_1/_2/_3 多合一包）改候选收集后 `sort.Strings` 取字典序最小——map 迭代随机曾致同输入不同运行绑不同纹理（与 parse.go geometry.* 键选取同口径，测试 50 次一致性锁）；② `detectMaidNs` 从「条目序首个 maid_model.json」改为复用 `collectMaidManifest`（"最长清单即主包"）——多命名空间包组件视图与合并预览选 ns 不再分叉，空 logPrefix 不打日志；③ 组件路径排序键归一化：`buildOrderAndPngIndex`/`sortGeoFilesMainFirst` 双侧加 ToLower（此前仅合并版 sortByModelOrder 归一，混合大小写 zip 声明序静默失效）；④ `trimTexExt` 收敛全文 7 处 TrimSuffix(.png/.jpg) 内联；`buildComponents` 恒 nil error 装饰性签名去除。行为锁 `archive_review_fix_test.go`
