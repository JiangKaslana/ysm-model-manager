...
| # | 类型 | 位置 | 一句话 | 价值 |
|---|------|------|--------|------|
| 1 | ①+⑥ | `frontend/src/utils/3d/model3d.ts` 全文 | RenderSession 完整对象化（camera/renderer/controls/container 收敛 + 16 回调方法化，~数百处引用）——**已裁决独立立项待启动，勿在常规轮次内 rush**（陷阱 #11 高危） | 高 |
| 2 | ⑥ | `go/importer/importer_file.go:122` + `go/packs/mcmeta.go:138` | 两套资源类型检测器并存，均应注册表驱动；新增类型须改两处 | 高 |
| 3 | ③ | `go/sync/sync_dirlevel.go:37` / `sync_push.go:29,70,80` / `instance.go:138` / `installer.go:281` | 文件夹级/目录型 rtype 判定 6+ 处硬编码，应改用 registry `isDir` | 高 |
| 4 | ② | `go/fsutil/` `copyFile×6` / `copyDirRecursive×4` | 原子复制原语成对重复，应收敛进 fsutil | 高 |
| 5 | ③ | `go/types/extensions.go:61` `ShouldHashExt` + scanner 内嵌 CI 清单 | 哈希白名单硬编码 switch，文件头声称注册表驱动唯独此处例外；新增类型须改 2 处 Go + 1 处 CI | 高 |
| 6 | ⑥ | `frontend/src/backend/browser-adapter.ts:69-202` | 40+ binding 手写大对象字面量，应改 binding 注册表（各职责模块自注册 + 元数据门控） | 高 |
| 7 | ② | `frontend/src/features/import-dnd.ts:50-72` 与 `import-queue-events.ts:83-115,174-199,225-247` | 网页版导入块在 4 处近逐字重复，已有细微漂移（folderInput 分支缺 stats:refresh） | 高 |
| 8 | ② | `frontend/src/app-modules.ts:28-47,115-118` | 5 个 Web Component 动态加载 catch 模板逐字重复，应抽 `loadView(name, importFn)` | 高 |
| 9 | ④ | `go/types/resource.go:18-32` `ResourceType` | 纯数据无 Go 层 hook 字段，importer/packs 的扩展点无法从 JSON 表达；增加 `plugin`/`handlerRef` 字段可打通三处硬编码根 | 高 |
...
### 4.1 `features/import-queue-data.ts:157-238` + `events.ts:294-452` — ②未抽离重复
...
### 4.2 `features/import-executor.ts:182` + `import-queue-events.ts:387` — ⑥可插件化
- 冲突判定靠 `errMsg.includes("FILE_EXISTS") || includes("目标已存在")` 字符串匹配（两处，中文文案也重复）→ 统一消费 `friendlyError` 的结构化 `AppError.Code`（中，会随 Go 文案漂移）。

### 4.3 `features/oldest-models.ts:42-51,317-323` + `recycle-bin.ts:96-106` — ②未抽离重复（跨文件）
...
### 6.1 `go/importer/importer_file.go:122` + `go/packs/mcmeta.go:138` — ⑥可插件化（Top 2）
- 两套资源类型检测器并存 → 注册表驱动。

### 6.2 `go/sync/sync_dirlevel.go:37` / `sync_push.go:29,70,80` / `instance.go:138` / `installer.go:281` — ③缺扩展点（Top 3）
- 文件夹级/目录型 rtype 判定 6+ 处硬编码 → registry `isDir`。

### 6.3 `go/fsutil/` `copyFile×6` / `copyDirRecursive×4` — ②未抽离重复（Top 4）
- 原子复制原语成对重复 → 收敛进 fsutil。

### 6.4 `go/types/extensions.go:61` `ShouldHashExt` + scanner 内嵌 CI — ③缺扩展点（Top 5）
- 哈希白名单硬编码 switch，文件头声称注册表驱动唯独此处例外 → `ResourceType` 增加 `hashable`/`hashOnScan` 字段。

### 6.5 `go/types/resource.go:18-32` `ResourceType` — ④未充分利用抽象（Top 9）
- 纯数据无 Go 层 hook 字段 → 增加 `plugin`/`handlerRef` 字段打通 importer/packs/sync 三处硬编码根。

### 6.6 `go/types/extensions.go:16` — ①硬编码
- `MaxImportSize=500MB` 被 scanner/download/importer 三方引用 → AppConfig 可配置（中）。

### 6.7 `go/types/config.go:4-28` `AppConfig` — ⑤缺配置项
- 已含 Mirror/VoxelMaxBlocks/LinkMode，但扫描 TTL/日志上限/预览读取上限/下载超时未暴露 → 集中下沉多处阈值（中）。

### 6.8 `go/dedup.go:40/149` / `recycle.go:64/223` / `instance.go:76-135` — ②未抽离重复
- 同文件内成对重复逻辑 → 见子代理汇报（中）。

### 6.9 `go/updater/updater.go:22-25,115-122,503-531` — ①④
- `repoOwner/repoName` 编译期常量、资产命名模式硬编码、手写 semver 比较无预发布语义 → 见子代理汇报（低）。

### 6.10 `go/logs/logs.go:17-27` / `runtime.go:11` — ①硬编码
- `maxLogEntries=500`/`maxFieldLen=1024`/`corruptRetentionDays=7`/`DefaultRuntimeCap=200` → 配置项（低）。

### 6.11 `go/threejs/spec.go:113-120,58-105` — ①④
