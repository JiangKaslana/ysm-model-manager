...
| `applyVPDToMesh()` | `frontend/src/utils/3d/adapters/mmd-adapter:1004` | Worker 路径下的 VPD 姿势应用： 复刻 applyVPD() 的核心逻辑（坐标转换 + 骨骼变换 + morph 影响）， 但不依赖 MMDLoader 产出的完整 MM |
| `MmdMenuItemsOpts()` | `frontend/src/utils/3d/adapters/mmd-adapter:1045` | mmdMenuItems 组装依赖：适配器 build 内组装；测试可构造假依赖遍历真实菜单表 |
| `mmdMenuItems()` | `frontend/src/utils/3d/adapters/mmd-adapter:1072` | MMD 声明式根菜单专属项（ADR-076 v2 Phase 2）：model / 材质 / 播放（+ 条件 bones）。 |
...
| `fitCameraToRoots()` | `frontend/src/utils/3d/camera-setup:57` | 按给定根节点列表（多模型同框）计算并集包围盒并返回相机初始位姿。 |
