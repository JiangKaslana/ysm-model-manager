...
| `GroupLabel()` | `go/types/extensions:359` | GroupLabel 返回分组显示名（从注册表各类型的 groupLabel 字段派生，消除 resourceGroups 冗余源）； 取该组第一个有 groupLabel 的类型 |
| `GroupIcon()` | `go/types/extensions:373` | GroupIcon 返回分组图标（从注册表各类型的 groupIcon 字段派生）。 |
| `SubDirMap()` | `go/types/extensions:393` | SubDirMap 返回指定资源类型在整合包实例版本目录中的扫描子目录 |
| `SubDirAll()` | `go/types/extensions:405` | SubDirAll 返回所有资源类型在整合包实例中的版本扫描子目录映射 |
| `AllSubDirs()` | `go/types/extensions:417` | AllSubDirs 返回所有资源类型的版本子目录信息（遍历用） |
| `SubDirEntry()` | `go/types/extensions:387` | SubDirEntry 资源类型的版本子目录信息 |
...
| `FormatRange.UnmarshalJSON()` | `go/types/resource:297` | UnmarshalJSON 实现 json.Unmarshaler，支持 int / [int] / [int,int] 三种格式 |
| `PackMeta.Desc()` | `go/types/resource:393` | Desc 返回 description 的可读文本（处理 string / JSON text component 对象 / 数组） |
...
| `FormatRange()` | `go/types/resource:291` | FormatRange 资源包 supported_formats 范围（可为 int 或 [int,int]） |
| `PackMeta()` | `go/types/resource:382` | PackMeta 资源包信息（来自 pack.mcmeta） |
| `LitematicMeta()` | `go/types/resource:400` | LitematicMeta 投影文件元数据（对应 .litematic 中 Metadata compound） |
| `LitematicBlockStat()` | `go/types/resource:417` | LitematicBlockStat 方块类型统计 |
| `LitematicVoxelData()` | `go/types/resource:423` | LitematicVoxelData 体素渲染数据 |
| `VoxelGroup()` | `go/types/resource:431` | VoxelGroup 同一颜色的方块组 |
...
| `bindTreeDnD()` | `frontend/src/features/import-dnd:133` | 在目标容器上注册仓库页 DnD 事件。 |
...
| `importWebFilesWithToast()` | `frontend/src/features/import-executor:230` | 网页版导入执行（ADR-049 Phase 3）：拖入/选择文件 → importWebFiles 直写 IndexedDB → toast 反馈 → tree/stats 刷新。 |
