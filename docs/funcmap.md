...
| `FindGeometryInExtractedYSM()` | `go/ysm/extracted:223` | FindGeometryInExtractedYSM 在解压后的 YSM 模型目录中查找 geometry 和纹理 ysmJsonPath: ysm.json 的完整路径 返回: |
| `FindComponentsInExtractedYSM()` | `go/ysm/extracted:497` | FindComponentsInExtractedYSM 多组件解析（YSMViewer 式）：解压目录内每个模型文件独立组件， **不合并 bones、不排除 arm**（arm |
