...
| `ParseFromZip()` | `go/geometry/archive:1324` | ParseFromZip 从 ZIP 字节中解析 Bedrock Geometry 并提取纹理和动画。 |
| `ParseFrom7z()` | `go/geometry/archive:1335` | ParseFrom7z 从 7z 字节中解析 Bedrock Geometry 并提取纹理。 |
| `ParseFromZipEntry()` | `go/geometry/archive:1354` | ParseFromZipEntry 按 subPath（zip 内路径，L0 SubModel.SourcePath 口径）解析单个 geometry 文件。 |
| `ParseFrom7zEntry()` | `go/geometry/archive:1380` | ParseFrom7zEntry 对应 ParseFromZipEntry 的 7z 版本；subPath 匹配策略完全一致。 |
| `IsMainModelName()` | `go/geometry/archive:1467` | IsMainModelName 判断模型文件是否为主组件（main.json / main.geo.json）。 |
| `ParseComponentsFromZip()` | `go/geometry/archive:1479` | ParseComponentsFromZip 多组件解析（YSMViewer 式）：zip 内每个模型文件独立组件， 含 arm/载具等组件（不合并、不排除）；main 优先排序， |
| `ParseComponentsFrom7z()` | `go/geometry/archive:1596` | ParseComponentsFrom7z 多组件解析（7z 版）：与 ParseComponentsFromZip 同构， 复用 collectArchiveFiles/buil |
...
| `PullResources()` | `go/sync/sync_push:74` | PullResources 拉取整合包多余资源回仓库 持 InstallLock：从实例目录复制文件回仓库，与 SyncToggleStatus/RelinkDir 等并发操作同一 |
| `PullSingleResource()` | `go/sync/sync_push:174` | PullSingleResource 拉取单个资源（文件夹/文件）回仓库 持 InstallLock：从实例目录复制文件回仓库，与并发同步操作互斥（ADR-056） |
| `PushSingleResource()` | `go/sync/sync_push:212` | PushSingleResource 推送单个资源到整合包： 文件夹 / .json/.pmx/.pmd（文件夹级类型）走 InstallDir，其余 Install。 |
...
| `SubModel()` | `go/types/bedrock:27` | SubModel 子模型条目（多角色加载）。 |
| `Bone2D()` | `go/types/bedrock:34` | Bone2D 骨骼简化信息（只用于 2D 线条图） |
| `Cube2D()` | `go/types/bedrock:44` | Cube2D 立方体信息 |
| `YsmMetadata()` | `go/types/bedrock:62` | YsmMetadata ysm.json 的 metadata 段（模型详情：名称/许可/作者/链接）。 |
| `YsmLicense()` | `go/types/bedrock:71` | YsmLicense 许可信息（wine_fox：{"type": "CC BY-NC-SA 4.0"}） |
| `YsmAuthor()` | `go/types/bedrock:77` | YsmAuthor 作者条目 |
...
| `litematicMenuItems()` | `frontend/src/utils/3d/adapters/litematic-adapter:379` | 构造 litematic 专属菜单项： 分层切片调节（axis/layer 控件）作为 🧍 模型组的一个面板项， 点击后弹出面板，内含轴选择 + 分层模式 + 滑块控件。 |
...
| `YsmAdapterOptions()` | `frontend/src/utils/3d/adapters/ysm-adapter:41` | 适配器可选项：loader 注入（预览面板语境数据加载链）/ 纹理重建 / 关闭回调 |
| `buildYsmScene()` | `frontend/src/utils/3d/adapters/ysm-adapter:101` | 构建 YSM 3D 内容并挂载到统一外壳（shared 模式）。 |
| `makeYsmAdapter()` | `frontend/src/utils/3d/adapters/ysm-adapter:364` | 工厂：构造统一 PreviewAdapter（shared 模式） |
| `YsmBonePanelRef()` | `frontend/src/utils/3d/adapters/ysm-adapter:378` | 骨骼面板清理引用（菜单项 render 与 adapter dispose 共享，防重入泄漏） |
| `YsmMenuItemsOpts()` | `frontend/src/utils/3d/adapters/ysm-adapter:383` | ysmMenuItems 组装依赖：适配器 build 内组装；测试可构造假依赖遍历真实菜单表 |
| `ysmMenuItems()` | `frontend/src/utils/3d/adapters/ysm-adapter:417` | YSM 声明式根菜单专属项（ADR-076 v2 Phase 2）：model / 截图 / 骨骼。 |
...
| `ModelSpec()` | `frontend/src/views/app-preview/model3d-loader:23` | Go 返回的 3D spec（models 数组） |
| `loadTextures()` | `frontend/src/views/app-preview/model3d-loader:52` | 并行加载纹理 URL 列表，返回 THREE.Texture 数组（P0 优化：纹理缓存池，同 URL 复用） |
| `preloadModel()` | `frontend/src/views/app-preview/model3d-loader:154` | 预加载：spec 先行，纹理按全量清单加载（texArr 槽位 = cube texSlot 下标） |
...
| `scanModelsByType()` | `frontend/src/views/app-preview/preview-library:94` | 按资源类型（+可选子类型）扫描候选模型路径（轻量：GetRepoRoot + ScanModelEntriesFiltered， 复用文件树扫描缓存，不逐文件解析）。供 3D 内切 |
| `withPreviewExtras()` | `frontend/src/views/app-preview/preview-library:111` | 给 mount3D opts 注入「跨类型换角色」入口 + 按类型懒加载数据源。各 createXxx3D 统一经此接入 |
