...
| 🟡 中 P3 | [preview-skeleton.ts#L270-L281](../frontend/src/views/app-preview/skeleton.ts#L270-L281)、[preview-litematic-3d.ts#L28-L40](../frontend/src/utils/3d/adapters/litematic-3d.ts#L28-L40) 等 | 全屏 overlay 大量内联硬编码色（`#1a1b2e` / `#2a2b3e` / `rgba(…)` 数十处），违 ADR-005「CSS 全走变量」；两处 3D overlay 的 topBar/控件样式近乎逐行重复（jscpd 已报） | overlay 挂在 document.body 可用 `:root` 变量；抽公共 `fullscreen-overlay-css` + 工厂函数消重 |
...
| # | 位置 | 一句话 | 修复量 |
|---|------|--------|--------|
| P1-1 | [preview-litematic-3d.ts#L160-L176](../frontend/src/utils/3d/adapters/litematic-3d.ts#L160-L176) | 加载期 ESC 泄漏整个 Three.js 场景 + rAF + 6 组监听 | 加 `aborted` 标志，await 后检查 |
