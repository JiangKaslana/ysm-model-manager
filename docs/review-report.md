...
> **注意**：本报告为历史审计记录（2026-08-03），部分代码引用（如 `import-queue.ts`）已因后续重构而失效，
> 但审计结论与模式分析仍具参考价值。
...
| 🟠 高 P2 | [handler-sync.ts#L40](../frontend/src/core/handlers/sync.ts#L40) | `const rtypeActual = rtype \|\| "ysm"` 硬编码字面量兜底，违 ADR-010 注册表优先（resource_types.json 单一事实来源）；上游漏传 rtype 时静默降级为 ysm 而非报错 ✅ **2026-08-29 已修复**（08616fc6）：bus.ts 契约 `rtype` 必填化（编译期堵漏）+ handler 入口 runtime 显式失败（error toast + done skipped；守卫置于 try 块内保证 finally 兜底解锁与回调，避免 busy 永锁）；字面量兜底已删 | 缺省即抛错/toast（参数契约错误应显式失败），或从 `RESOURCE_TYPES` 常量取值并注释决策 |
