...
| 5 | **errors 正则表收尾（ADR-051）** | 前端 friendlyError 正则模式表应在 Go 端结构化错误码改造完成后删除（当前「Code 优先 + 正则兜底」混合态）；删除需同步更新 errors.test.ts 的 16 类正则断言（`"too many open files"` 断言与 Go 端收窄口径不一致） | 2026-08-11 utils-errors 审计（已备案） |
