...
--bg:       /* 最底层背景 */ --surf: /* 表面背景（侧栏、顶栏） */
  --card: /* 卡片背景 */ --hover: /* hover / 选中底（导航栏与侧边栏选中态均用 --hover 淡底，亮色主题不加深） */
  --act: /* 保留变量，toast undo hover 等次要场景使用 */
  --accent: /* 强调色（链接、hover 高亮、关键按钮、焦点环、左侧选中指示条） */ --txt: /* 主文字色 */
  --muted: /* 次要文字色 */ --bd: /* 边框色 */;
```
...
| 强反馈对比 | 选中/激活靠淡底（--hover）+ 左侧 accent 细条区分，不加深背景色也不染色文字；hover 与 active 视觉差距仅靠边框，克制淡雅 |
...
> vitest 按 `src/**/*.test.{js,ts}` 自动发现（`vitest.config.ts`），**测试与源码同目录镜像**（`foo.ts` → `foo.test.ts` 同目录），不设独立测试目录。

| 后缀模式 | 语义 | 示例 |
|---------|------|------|
| `<mod>.test.ts` | 纯函数/模块级单测（与源码同名） | `render.test.ts` / `data.test.ts` |
| `<mod>.component.test.ts` | 组件级交互测试（G-1，断言 testid + 事件流） | `app-tree.component.test.ts` |
| `<mod>.methods.test.ts` | 组件方法级补充测试 | `app-content.methods.test.ts` |
| `<mod>.integration.test.ts` | 入口函数/集成链路测试（加载、初始化、事件委托全链路） | `recycle-bin.integration.test.ts` |
