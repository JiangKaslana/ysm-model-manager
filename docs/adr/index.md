...
> 架构决策日志，共 **68** 篇。决策真相源 = 各 ADR 文件首部「状态」行；本页为登记表 + 规范索引（单文件承载全部）。
...
| ADR-065 | 整合包侧资源类型语义收敛：rtype 分支注册表驱动单点 | 🔄 部分采纳（方向已定，编码待立项落地） | 2026-08-15 |
| ADR-066 | 全资源预览器：统一预览契约与注册表驱动分发 | 🔄 部分采纳（统一契约方向已定；**P0「硬编码派发墙」已于 `0615b21d` 落地**，D2–D5 待后续阶段） | 2026-08-16 |
| ADR-067 | zip 化资源识别：扩展名歧义消解与内容指纹覆盖 | 🔄 部分采纳（S4 前端契约本批落地；S1/S2 改动 Go 检测核心，待架构师确认后执行） | 2026-08-16 |
| ADR-068 | 统一容器桥接层：ContainerReader 抽象收敛 ysm/geometry/avatar 解包重复 | 🔄 部分采纳（方向已定，编码待立项落地） | 2026-08-16 |
