...
> 架构决策日志，共 **52** 篇。决策真相源 = 各 ADR 文件首部「状态」行；本页为登记表 + 规范索引（单文件承载全部）。
...
| ADR-049 | 网页版（Web 端）桥接：backend 适配器 + IndexedDB 模型库 | 🔄 部分采纳（功能全落地：Phase 0-3 + Web e2e + 部署配置；剩 3D 预览 WASM 专项 P2-2 与推送验证） | 2026-08-10 |
| ADR-050 | 模型广场 · 浏览器窗口（Wails 第二窗口） | ✅ 已采纳 | 2026-08-11 |
