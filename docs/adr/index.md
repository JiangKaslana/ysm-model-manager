...
> 架构决策日志，共 **55** 篇。决策真相源 = 各 ADR 文件首部「状态」行；本页为登记表 + 规范索引（单文件承载全部）。
...
| ADR-049 | 网页版（Web 端）桥接：backend 适配器 + IndexedDB 模型库 | 🔄 部分采纳（功能全落地：Phase 0-3 + P2-2 3D 渲染闭环 + Web e2e + 部署配置；剩线上 URL 验证） | 2026-08-10 |
