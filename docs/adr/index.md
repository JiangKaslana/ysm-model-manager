...
> 架构决策日志，共 **100** 篇。决策真相源 = 各 ADR 文件首部「状态」行；本页为登记表 + 规范索引（单文件承载全部）。
...
| ADR-092 | 资源类型分组（Group）分层路由：Minecraft / Minecraft-Mod / MMD 总目录归并 | 🔄 部分采纳（group 层第 1 层已落地；MMD 子类型化为后续） | 2026-08-18 |
