...
> 架构决策日志，共 **85** 篇。决策真相源 = 各 ADR 文件首部「状态」行；本页为登记表 + 规范索引（单文件承载全部）。
...
| ADR-077 | 底部导航通用外壳收敛（D1+D3 落地） | 🔄 部分采纳（D1 编码立项，D3 相机控件同步下沉） | 2026-08-16 |
...
| ADR-087 | AI 自动化取巧——pre-commit 智能 stage 与无脑指令下沉 | 🔄 部分采纳 | 2026-08-17 |
