...
> 架构决策日志，共 **71** 篇。决策真相源 = 各 ADR 文件首部「状态」行；本页为登记表 + 规范索引（单文件承载全部）。
...
| ADR-071 | 网页版能力边界：.7z 明确不支持 + 社区站点编辑保存补齐 | 🔄 部分采纳（方向已定，编码待立项落地） | 2026-08-16 |
| ADR-072 | 3D 代码归置与预览派发注册表化：适配器下沉 utils/3d/adapters | 🔄 部分采纳（方向已定，编码待立项落地——等 ADR-070/071 web 端施工稳定后动工，避免与施工中地基冲突） | 2026-08-16 |
