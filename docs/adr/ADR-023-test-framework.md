...
- **配置**：`frontend/vitest.config.ts` test 块——`environment: "happy-dom"`、`setupFiles: ["./test-setup.ts"]`、`coverage: { provider: "v8", reporter: ["text","html","json"], include: ["src/**/*.ts","src/**/*.js"], exclude: ["src/**/*.test.{js,ts}","src/wasm/**","src/views/app-preview/wasm.ts"] }`；
...
设计取舍：
- **不塞进 `pages-deploy.yml`**：该 job 是纯文档站 VitePress 部署，无 node_modules 环境；vitest 归主 CI job，职责分离。
