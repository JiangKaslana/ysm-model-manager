...
- **MMD 资源**：`mmd-skin` 目录结构**⚠️ 待上游源码核实**：本 ADR 初稿按用户口述记为「5 子目录（pmx 模型 / pmx 场景 / vmd 动作 / mp3 音频 / 第 5 类待确认）」，但已知分析文档 `docs/archive/reference/mmdskin-analysis.md` 记录的真实 MmdSkin 结构为**单模型文件夹**（含 `model.pmx` + 纹理 + `.vmd` 动作 + `.vpd` 表情），**无 mp3、非 5 子目录**。两者冲突，详见 §4 溯源与勘误。**禁止据此实现。**
- **VRM**：`vrchat-avatar` 用户明确「先独立」，不参与归并。
...
3. **分组归并映射**：
   ...
   - `mmd`：`mmd-skin`，其物理子目录结构**⚠️ 冻结待核实（见 §4 勘误）**：初稿假设 `subFolders` = `models/pmx`、`scenes/pmx`、`motions/vmd`、`audio/mp3`、第 5 类；但真实 MmdSkin 源码为单模型文件夹混放（`model.pmx` + `.png` + `.vmd` + `.vpd`），**无 mp3、无分离子目录**。**在拉取上游源码 `shiroha-233/MC-MMD-rust` 核实前，`subFolders` 字段与 mp3 假设一律冻结、不得落地实现。**
...
- **负面 / 改动面**：两级路径影响 3 处落盘 / 读取点——`internal/app/app.go:152-155`（MkdirAll）、`go/types` 路径解析、`frontend/src/views/app-content/settings/path-cards.ts:225`（`filesRoot + "/" + storageSubDir`）、以及 sync 模块；需统一改走 `GroupStorageRoot`。
- **风险与缓解**：① 存量单级目录兼容性——靠 `GroupStorageRoot` 回退兜底，不触发 mass-move；② `maid-model` 注册需补 full schema（扩展名 / 检测器），否则扫描不到；③ 第 5 个 MMD 子目录未确认前，`subFolders` 先填 4 项，留 1 项待补。
