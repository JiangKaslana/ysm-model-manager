...
- **已接入 CI**（2026-08-10 更新）：E2E 已接入 `test.yml` e2e job（ubuntu + 浏览器缓存 + `install --with-deps chromium` + webServer 自动起 vite dev，参照 MikuMikuAR 实证模式）。参数：全局 7 分钟总限（`globalTimeout`）/ 每用例 15s（`timeout`）/ 累计 5 次失败阻断（`maxFailures=5`）＋ `retries=1`。首批接入时修复了 mock 契约漂移（`ScanModelEntries`→`ScanModelEntriesWithLabel`、`GetInstanceSyncStatus`→`GetResourceInstanceStatus`，见 `frontend/e2e/mock-data.ts`）。
- **覆盖现状**（2026-08-10 复核，`npx playwright test --list` 实测）：**15 spec 文件 / 51 用例**（`frontend/e2e/*.spec.ts`），覆盖导航切换 / 文件树浏览 / 右键菜单 / 拖拽导入 / 弹窗（modal）/ 多选 / 搜索 / toast / 设置 / 资源管理器 / 同步管理器 / 工坊 / 诊断 / 侧边菜单。mock bridge 走 `frontend/e2e/mock-data.ts`（与 vitest mock 双源，漂移由契约测试守护）。3D 渲染 / Wails 原生能力 / 真实网络仍按 §2.5 排除。
